...
// @include        https://www.neopets.com/island/fight_training.phtml?type=status
...
(function () {
  var list = xpath(
    ".//td[@class = 'content']//td[not(form) and b[contains(text(), 'minute')]]/b/text()"
  );
  ...
  for (var ai in list) {
    ...
    window.setInterval(
      function (obj) {
        obj.node.textContent = obj.timer.toString(
          "{0?(- }{1} hr{1?s}, {2} minute{2?s}, {3} second{3?s}{0?)}"
        );
        ...
        if (obj.timer.current(true) < 0 && !obj.complete) {
          ...
          if (obj.pet) {
            var f = obj.node.parentNode.parentNode.appendChild(
              document.createElement("form")
            );
            f.setAttribute("method", "post");
            f.setAttribute("action", "process_fight_training.phtml");
            f.innerHTML =
              '<input type="hidden" value="complete" name="type" /><input type="hidden" value="' +
              obj.pet +
              '" name="pet_name" /><input type="submit" value="Complete Course!" />';
          }
        }
      },
      1000,
      {
        ...
        pet:
          /\/cpn\/(.+?)\//.test(
            xpath("string(./ancestor::tr[1]/td[1]/img/@src)", o)
          ) && RegExp.$1,
