...
// @icon           https://gm.wesley.eti.br/icon.php?desc=34084
...
(function recursive() {
  // script scope
  var last = Date.parse(
      GM_getValue("lastAccess", "Mon Jun 20 2011 20:40:43 GMT-0300")
    ),
    curr = new Date(),
  ...
  if (
    last != curr.valueOf() &&
    /^(?:6am|2pm|10pm)$/.test(unsafeWindow.nh + unsafeWindow.na)
  ) {
    GM_setValue("lastAccess", (last = curr).toString());
    GM_openInTab("https://www.neopets.com/winter/snowager.phtml");
  }
