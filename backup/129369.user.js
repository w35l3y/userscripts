...
// @include        https://www.neopets.com/objects.phtml?type=shop*
// @include        https://www.neopets.com/haggle.phtml*
// @include        https://www.neopets.com/auctions.phtml?type=bids&auction_id=*
// @include        https://www.neopets.com/auctions.phtml
// @include        https://www.neopets.com/pound/adopt.phtml
// @include        https://www.neopets.com/pound/abandon.phtml
// @grant          GM_getValue
// @grant          GM_setValue
// @grant          GM_openInTab
...
// @icon           https://gm.wesley.eti.br/icon.php?desc=129369
...
// @history        2.0.0 Added <a href="https://userscripts-mirror.org/guides/773">Includes Checker</a>
...
(function () {
  // script scope
  var np = parseInt(
      xpath("string(id('npanchor')/text())").replace(/\D+/g, ""),
      10
    ),
    opt = JSON.parse(
      GM_getValue(
    ...
    page = {
      "/browseshop.phtml": {
        ...
        price: function (obj) {
          return (
            (/old_price=(\d+)/.test(obj.target.href) ||
              (obj.target.onclick &&
                /(\d+)NP/i.test(
                  obj.target.onclick
                    .toSource()
                    .replace(/(?:[\seoit,.]|\\u[\da-f]{4})+/gi, "")
                ))) &&
            RegExp.$1
          );
        },
      ...
      "/auctions.phtml": [
        {
          ...
          price: function (obj) {
            return xpath(
              "string(./ancestor::tr[1]/td[6]/b/text())",
              obj.target
            );
          },
        },
      ...
      "/pound/abandon.phtml": {
        ...
        confirm: function (obj) {
          var c = xpath(
            "string(.//input[@name = 'confirm']/@value)",
            obj.target.form
          );
    ...
    (withdrawAndBuy = function (e, cb) {
      ...
      if (price >= np && opt.type) {
        if (0x001 == opt.type) {
          ...
          page.confirm.apply(page, [obj]) &&
          0x002 & opt.type &&
          Pin.execute("pin_request", page.pin && page.pin.apply(page, [obj]))
        ) {
          console.log(1);
          Bank.withdraw({
            ...
            onsuccess: function (xhr) {
              ...
              if (!xhr.error || xhr.message) {
                ...
                if ((price > np && 0x001 & opt.type) || msg) {
                  ...
                  send(
                    msg ||
                      "You don't have enough neopoints on hand. (" + np + " NP)"
                  );
