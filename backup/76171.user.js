...
// @icon           https://gm.wesley.eti.br/icon.php?desc=76171
// @require        https://github.com/w35l3y/userscripts/raw/master/includes/Includes_XPath/63808.user.js
// @require        https://github.com/w35l3y/userscripts/raw/master/includes/Includes_Neopets/63810.user.js
// @contributor    jellyneo (https://twitter.com/statuses/user_timeline/42098834.rss)
// ==/UserScript==
...
var user = xpath(
  "string(id('header')//a[contains(@href, 'userlookup')]/text())"
);

if (/^(?:.+)$/.test(user)) {
  var prize = xpath(
    ".//a[contains(@id, 'tarla_prize_link_')]",
  ...
  else if (xpath("boolean(id('tarla2010-pushdown'))")) {
    // maxed out the number of prizes for today
    var next = new Date(),
      curr = Neopets.convert(document).Time(true);
    next.setHours(24 * (next.getDate() == curr.getDate()), 0, 0, 0);
    GM_setValue(
      "last_access-" + user,
      new Date(next - curr + new Date().valueOf()).toString()
    );
  }
  ...
  (function recursive() {
    var current = new Date(),
      last_access = new Date(
        Date.parse(
          GM_getValue("last_access-" + user, "May 5 2010 00:00:00 GMT-0300")
        )
      ),
    ...
    if (current - last_access > interval) {
      ...
      GM_xmlhttpRequest({
        method: "get",
        url: "https://twitter.com/statuses/user_timeline/42098834.rss",
        onload: function (xhr) {
          var title = xpath(
              ".//item/title[contains(translate(text(), 'ALRT', 'alrt'), 'tarla') and contains(text(), 'http')]",
              new DOMParser().parseFromString(xhr.responseText, "text/xml")
            )[0],
            current = new Date();
          if (
            title &&
            title.textContent != GM_getValue("sighting-" + user) &&
            /(https?:\/\/\S+)/.test(title.textContent)
          ) {
            ...
            if (
              current -
                Date.parse(
                  xpath("string(./ancestor::item[1]/pubDate)", title)
                ) <
              1200000
            )
              // current - pubDate < 20 minutes
...
GM_registerMenuCommand(
  "[Neopets : Tarla's Tour of Mystery 2010] Toggle auto-click on prizes",
  function () {
