...
// @icon           https://gm.wesley.eti.br/icon.php?desc=77897
...
// @contributor    jellyneo (https://www.jellyneo.net/?go=plotsummary_ac)
// @history        3.0.0 Added <a href="https://userscripts-mirror.org/guides/773">Includes Checker</a>
...
if (
  xpath(
    "string(id('content')//td//a[contains(@href, 'boardlist') and contains(text(), 'Altador')]/@href)"
  )
) {
  var teams = [
      "Unknown",
  ...
  function calculatedRank(text) {
    if (/(\w+)_(engrave|\w+gem)\.png/.test(text)) {
      var groups = ["wood", "stone", "bronze", "silver", "gold"].indexOf(
          RegExp.$1
        ),
        gems = ["yellowgem", "redgem", "greengem", "bluegem"].indexOf(
          RegExp.$2
        );
    ...
    var ranks = [
        50, 100, 150, 200, 275, 350, 425, 500, 600, 700, 800, 900, 1025, 1150,
        1275, 1400, 1550, 1700, 1850, 2000,
      ],
    ...
    for (
      var v,
        re =
          /<td style='text-align: right; font-weight: bold;'>(\d+(?:[,.]\d{3})*)<\/td>/gi;
      (v = re.exec(text));
      ++ai
    ) {
      if (
        ai in indexes &&
        (sum += parseInt(v[1].replace(/[,.]+/g, ""), 10) * (1 / indexes[ai])) >=
          ranks[at - 1]
      ) {
        return at;
      }
  ...
  function ac(p, l, yr, isCache, hc) {
    ...
    if (yr in info.cups && "team" in info.cups[yr]) {
        ...
        for (var ai in positions[yr]) {
          if (positions[yr][ai] == info.cups[yr].team) {
            pos =
              ...
              ++ai +
              ordpos[
                !/1[12]$/.test(ai) && /(\d)$/.test(ai) && RegExp.$1 < 4
                  ? RegExp.$1
                  : 0
              ] +
              " " +
      ...
      if (history) {
        history.innerHTML +=
          '<a href="https://www.neopets.com/altador/colosseum/' +
          yr +
          "/trophy.phtml?username=" +
          l +
          '" target="_blank"><img height="20" style="border: 1px solid #000000;" alt="' +
          teams[info.cups[yr].team] +
          '" title="' +
          rank2str(info.cups[yr].rank) +
          pos +
          '" src="https://images.neopets.com/altador/altadorcup/team_logos/' +
          teams[info.cups[yr].team].replace(/\s+/g, "").toLowerCase() +
          '_50.gif" /></a> ';
        //				history.innerHTML += '<a href="https://www.neopets.com/altador/colosseum/' + yr + '/trophy.phtml?username=' + l + '" target="_blank"><img height="20" style="border: 1px solid #000000;padding: 1px" alt="' + teams[info.cups[yr].team] + '" title="' + rank2str(info.cups[yr].rank) + pos + '" src="https://images.neopets.com/neoboards/smilies/' + teams[info.cups[yr].team].replace(/\s+/g, '').toLowerCase() + '.gif" /></a> ';
        //				history.innerHTML += '<a href="https://www.neopets.com/altador/colosseum/' + yr + '/trophy.phtml?username=' + l + '" target="_blank"><img width="22" height="22" style="border: 1px solid #000000;" alt="' + teams[info.cups[yr].team] + '" title="' + rank2str(info.cups[yr].rank) + pos + '" src="https://images.neopets.com/altador/altadorcup/team_logos/' + teams[info.cups[yr].team].replace(/\s+/g, '').toLowerCase() + '_50.gif" /></a> ';
      } else {
        ...
        hr.firstChild.innerHTML =
          '<hr color="#d1d1d1" size="1" noshade="noshade" />';
        p.appendChild(hr);

        acx.appendChild(document.createElement("td"));
        acx.firstChild.innerHTML =
          '<a href="https://www.neopets.com/altador/colosseum/' +
          yr +
          "/trophy.phtml?username=" +
          l +
          '" target="_blank"><img width="50" height="50" style="border: 1px solid #' +
          (yr == cyear ? "00" : "FF") +
          '0000;" alt="' +
          teams[info.cups[yr].team] +
          '" title="' +
        ...
        acx.childNodes[1].innerHTML =
          "<b>" +
          teams[info.cups[yr].team] +
          "</b><br />" +
          rank2str(info.cups[yr].rank) +
          '<div style="margin-top: 4px;" class="cup_history"></div>';
        //				acx.childNodes[1].innerHTML = '<img width="50" height="50" alt="' + rank2str(info.cups[yr].rank) + '" title="Level ' + info[yr].rank + '" src="https://images.neopets.com/altador/altadorcup/' + yr + '/badges/' + info.cups[yr].rank + '.png" />';
        p.appendChild(acx);
      }
    }
  ...
  function init() {
    ...
    xpath(
      "id('boards_table')//td//td/a[contains(@href, 'userlookup')]"
      ...
      if (login) {
        if (
          login in users &&
          current - cache_time < Date.parse(users[login].update)
        ) {
          ...
          img.setAttribute(
            "src",
            "https://www.obyneo.net/ac/2013/calc.png?u=" + login
          );
      ...
      if (!list.length) {
        if (cyear - yr < history_cups) {
          setTimeout(
            ...
            777 + 477 * Math.random(),
            yr - 1,
            JSON.parse(update_users.toSource())
          );
        } else {
        ...
        HttpRequest.open({
          method: "get",
          url:
            "https://www.neopets.com/altador/colosseum/" + yr + "/trophy.phtml",
          headers: {
            Referer: "https://www.neopets.com/userlookup.phtml?user=" + list[0],
          },
          onsuccess: function (xhr) {
            ...
            if (t) {
              users[l].cups[yr] = {
                ...
                rank: parseInt(
                  (/\/(?:badges\/|levelshields_?\d+_)(\d+)\./.test(
                    xhr.response.text
                  ) &&
                    RegExp.$1) ||
                    calculatedRank(xhr.response.text),
                  10
                ),
              };
  ...
  if (upd_positions) {
    HttpRequest.open({
      url: "https://bookofages.jellyneo.net/events/altador-cup/",
      method: "get",
      headers: {
        Referer: "https://www.jellyneo.net/?go=altadorcup11",
      },
      onsuccess: function (xhr) {
        var yr = 2005,
          iteams = {},
