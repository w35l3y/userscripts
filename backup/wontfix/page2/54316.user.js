...
// @version        1.0.2.0
// @language       en
// @contributor    Steinn (https://userscripts-mirror.org/users/85134)
// @include        nowhere
// @grant          GM_log
// @grant          GM_addStyle
// @grant          GM_getValue
...
// @icon           https://gm.wesley.eti.br/icon.php?desc=54316
...
Inventory.list = function (onLoadCallback) {
  if (typeof onLoadCallback != "function")
    ...
    xargs.unshift(
      "GET",
      "https://www.neopets.com/objects.phtml?type=inventory",
      function (e) {
        var sitems = [];
        var items = e.responseXML.evaluate(
          "//td[@class='content']//table//td[a/img]",
          e.responseXML,
          null,
        ...
        for (var ai = 0, at = items.snapshotLength; ai < at; ++ai) {
          ...
          var link = "https://www.neopets.com";
          var index = 0;
          if (
            /^cash_win/i.test(
              item[0].getAttribute("onclick") || item[2].getAttribute("onclick")
            )
          ) {
          ...
          switch (
            (item[0].parentNode.style.backgroundColor || "").replace(
              /^rgb|[\s()]+/g,
              ""
            )
          ) {
          ...
          for (
            var bi = 4 + index;
            item[bi] && /^span$/i.test(item[bi].tagName);
            bi += 2
          ) {
            var font = item[bi].getElementsByTagName("font");
            var color = (
              (font.length && font[0].getAttribute("color").toLowerCase()) ||
              item[bi].style.color ||
              ""
            ).replace(/^rgb|[\s()]+/g, "");

            if (color in color_categories) cats |= color_categories[color];
          }
...
Inventory.auction = function (item, opts, onLoadCallback) {
  ...
  if (!item) alert("[Inventory.auction]\nArgument 1 is missing");
  else if (!("Parameters" in item) || !item.Parameters[0])
    alert("[Inventory.auction]\nArgument 1 must be a valid Item");
  else if (
    (item.Category &
      (Inventory.Category.Neocash |
        Inventory.Category.Trading |
        Inventory.Category.Auctioned)) !=
    0
  )
    ...
    } else xargs = array_slice(arguments, 3) || [];

    var req = new HttpRequest();
    //req.options.headers["Referer"] = item.Link;
    xargs.unshift(
      "POST",
      "https://www.neopets.com/add_auction.phtml",
      function (e) {
        if (typeof onLoadCallback == "function") {
          var msg = e.responseXML.evaluate(
            "//div[@class='errormess' and b]",
            e.responseXML,
            null,
...
Inventory.process = function (item, action, onLoadCallback) {
  ...
  if (!item) alert("[Inventory.process]\nArgument 1 is missing");
  else if (!("Parameters" in item) || !item.Parameters[0])
    alert("[Inventory.process]\nArgument 1 must be a valid Item");
  else if (!action) alert("[Inventory.process]\nArgument 2 is missing");
  else if (action == "give")
    alert("[Inventory.process]\nUse Inventory.give() instead");
  else if (action == "auction")
    alert("[Inventory.process]\nUse Inventory.auction() instead");
  else if (
    !/^(?:Equip |(?:Feed|Give) to |(?:(?:safetydeposit|closet|stockshop|stockgallery|donate|drop)$))/i.test(
      action
    )
  )
    alert("[Inventory.process]\nArgument 2 is unknown (" + action + ")");
  else if (
    (item.Category & Inventory.Category.Neocash) != 0 &&
    !/^(closet|stockgallery|safetydeposit)$/i.test(action)
  )
    alert(
    ...
    (item.Category &
      (Inventory.Category.Trading | Inventory.Category.Auctioned)) !=
    0
  )
    alert(
      "[Inventory.process]\nThis item cannot be processed. Try again later"
    );
  else if (item.Parameters[4]) {
    // is_giftbox ?
    alert("[Inventory.process]\nNot implemented yet");
    ...
    xargs.unshift(
      "POST",
      "https://www.neopets.com/process_cash_object.phtml",
      function (e) {
        if (typeof onLoadCallback == "function") {
          var msg = e.responseXML.evaluate(
            "//div[@class='errormess' and b]",
    ...
    xargs.unshift(
      "POST",
      "https://www.neopets.com/useobject.phtml",
      function (e) {
        if (typeof onLoadCallback == "function") {
          var msg = e.responseXML.evaluate(
            "//div[@class='errormess' and b]",
...
Inventory.give = function (item, to, onLoadCallback) {
  if (!item) alert("[Inventory.give]\nArgument 1 is missing");
  else if (!("Parameters" in item) || !item.Parameters[0])
    alert("[Inventory.give]\nArgument 1 must be a valid Item");
  else if (!to) alert("[Inventory.give]\nArgument 2 is missing");
  else if (
    (item.Category &
      (Inventory.Category.Neocash |
        Inventory.Category.Trading |
