...
// @namespace      https://gm.wesley.eti.br/inludes
...
// @icon           https://gm.wesley.eti.br/icon.php?desc=54163
...
Wizard.find = function (
  ...
  } else xargs = array_slice(arguments, 6) || [];

  if (!text) alert("[Shop Wizard]\nArgument 1 is missing");
  else if (typeof onLoadCallback != "function")
    alert("[Shop Wizard]\nCallback function is missing");
  else {
    var req = new HttpRequest();
    //req.options.headers['Referer'] = 'https://www.neopets.com/market.phtml?type=wizard';

    xargs.unshift("POST", "https://www.neopets.com/market.phtml", function (e) {
      ...
      for (var ai = 0, at = items.snapshotLength; ai < at; ++ai) {
        ...
        r.push({
          ...
          Link:
            (!/^http:\/\//i.test(href) ? "https://www.neopets.com" : "") +
            (!/^\//.test(href) ? "/" : "") +
            href,
      ...
      var msg = e.responseXML.evaluate(
        "//div[@class='errormess' and b] | //td[@class='content']/center[b[2]]",
        e.responseXML,
        null,
    ...
    req.send({
      ...
      criteria:
        typeof is_identical == "undefined" || is_identical
          ? "exact"
          : "containing",
