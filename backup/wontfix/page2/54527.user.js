...
// @icon           https://gm.wesley.eti.br/icon.php?desc=54527
...
Shop.fromDocument = function (xml) {
  var sitems = [],
    items = xml.evaluate(
      ".//td[@class = 'content']//table[1]//td/a[contains(@href, '&_ref_ck') and img]",
      xml,
      null,
  ...
  for (var ai = 0, at = items.snapshotLength; ai < at; ++ai) {
    ...
    if (/&obj_info_id=(\d+)/.test(item.href))
      sitems.push({
        Id: RegExp.$1,
        Link:
          (!/^http:\/\//i.test(item.href) ? "https://www.neopets.com" : "") +
          (!/^\//.test(item.href) ? "/" : "") +
          item.href,
        Image: img.src,
...
Shop.list = function (link, onLoadCallback) {
  ...
  if (!link) alert("[Shop.buy]\nArgument 1 is missing");
  else if (typeof onLoadCallback != "function")
    alert("[Shop.list]\nCallback function is missing");
  else if (link && link.responseXML) {
    var msg = link.responseXML.evaluate(
      ".//div[@class='errormess' and b] | .//td[@class='content']/p[5]/b | .//td[@class='content']/center/p[1]",
      link.responseXML,
      null,
    ...
    xargs.unshift(
      Shop.fromDocument(link.responseXML),
      link,
      msg && /^div$/i.test(msg.tagName),
      msg
    );
    onLoadCallback.apply(this, xargs);
  } else {
    var req = new HttpRequest();
    //req.options.headers["Referer"] = "https://www.neopets.com/browseshop.phtml";
    ...
    xargs.unshift("GET", link, function (e) {
      ...
      xargs.unshift(
        Shop.fromDocument(e.responseXML),
        e,
        msg && /^div$/i.test(msg.tagName),
        msg
      );
...
Shop.buy = function (item, onLoadCallback) {
  if (!item) alert("[Shop.buy]\nArgument 1 is missing");
    ...
    xargs.unshift("GET", item.Link, function (e) {
      ...
      //	Config...	https://www.neopets.com/browseshop.phtml
...
/*
Shop.list("https://www.neopets.com/browseshop.phtml?owner=...", function(list,e,h,m)
{
	var items = [];
	var buy;
