...
// @icon           https://gm.wesley.eti.br/icon.php?desc=54528
...
Course.fromDocument = function (xml) {
  var res = {}, // result
    pets = xml.evaluate(
      ".//td[@class='content']//table[1]//tr[position() mod 2 = 0]",
      xml,
      null,
  ...
  for (var ai = 0, at = pets.snapshotLength; ai < at; ++ai) {
    ...
    if (completeform.length) {
      status = 3;
      comp = /^complete$/i.test(
        completeform[0].elements.namedItem("type").value
      );
      ...
      for (var bi = 0, bt = itemsimg.length; bi < bt; ++bi) {
        var n =
          codestones_ids +
          parseInt(itemsimg[bi].src.match(/\/codestone(\d+)\.gif$/)[1], 10);

        if (n in sitems) ++sitems[n].Quantity;
        else
          sitems[n] = {
            Id: n,
...
Course.status = function (e, onLoadCallback) {
  ...
  } else xargs = array_slice(arguments, 2) || [];
  ...
  if (typeof onLoadCallback != "function")
    ...
    xargs.unshift(
      ...
      function (e) {
        ...
        xargs.unshift(
          Course.fromDocument(e.responseXML),
          e,
          /\?type=status$/.test(e.finaUrl),
          msg
        );
        onLoadCallback.apply(this, xargs);
      }
...
Course.start = function (type, pet, onLoadCallback) {
  if (!type) alert("[Course.start]\nArgument 1 is missing");
  else if (!pet) alert("[Course.start]\nArgument 2 is missing");
  else {
    var xargs = array_slice(arguments, 3) || [];

    var req = new HttpRequest();
    //req.options.headers["Referer"] = "https://www.neopets.com/island/training.phtml?type=courses";
    xargs.unshift(
      "POST",
      "https://www.neopets.com/island/process_training.phtml",
      function (e) {
        ...
        //	Config...	https://www.neopets.com/island/training.phtml?type=courses
...
Course.pay = function (pet, onLoadCallback) {
  if (!pet) alert("[Course.pay]\nArgument 1 is missing");
  else {
    var xargs = array_slice(arguments, 2) || [];

    var req = new HttpRequest();
    //req.options.headers["Referer"] = "https://www.neopets.com/island/training.phtml?type=status";
    xargs.unshift(
      "GET",
      "https://www.neopets.com/island/process_training.phtml?type=pay&pet_name=" +
        pet,
      function (e) {
        //	https://addons.mozilla.org/en-US/firefox/addon/10636
        ...
        //	Function	referrer to specified site
        //	Config...	https://www.neopets.com/island/training.phtml?type=status
