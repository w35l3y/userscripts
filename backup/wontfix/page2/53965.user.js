...
// @contributor    sizzlemctwizzle (https://userscripts-mirror.org/guides/9)
// ==/UserScript==
...
HttpRequest = function () {
  ...
  this.open = function (method, url, onLoadCallback) {
    ...
    this.options.onload = function (e) {
      if (
        /^Content-Type: (?:text|application)\/(?:x-)?json/m.test(
          e.responseHeaders
        )
      ) {
        ...
        if (/^Content-Type: text\/xml/m.test(e.responseHeaders)) {
          e.responseXML = new DOMParser().parseFromString(
            e.responseText,
            "text/xml"
          );
  ...
  this.send = function (content) {
    ...
    if (false) {
      ...
      unsafeWindow.TempXMLHttpRequest = (function (opt) {
        ...
        x.onload = function () {
          opt.onload({
            // simulates GM_xmlhttpRequest response object
            status: x.status,
            finalUrl: opt.url,
