...
// @icon           https://gm.wesley.eti.br/icon.php?desc=54000
// ==/UserScript==
...
Neopets = function (doc) {
  ...
  var number = function (xpath) {
    return (
      parseInt(
        ((string(xpath).match(/^\d+(?:[,.]\d{3})*/) || [])[0] || "").replace(
          /[,.]/g,
          ""
        ),
        10
  ...
  this.Time = function (is_dynamic) {
    ...
    var nst = (
      string("//script[contains(text(),'var nh = ')]/text()").match(
        /var n[hmsa] = ([""''])?[\w\d]{1,}\1\;$/gm
      ) ||
      (function (x /*dst*/) {
        var o = new Date(x);
        ...
        if (i <= o && o < f) o.setUTCMilliseconds(3600000);

        return (
          [
            ((o.getUTCHours() - 1) % 12) + 1,
            o.getUTCMinutes(),
            o.getUTCSeconds(),
            o.getUTCHours() > 11 ? "'pm'" : "'am'",
          ].join(";\n") + ";"
        ).split("\n");
      })(out)
    ).map(function (a) {
    ...
    for (var i in nst) sum -= parseInt(nst[i], 10) * [3600, 60, 1][i];
    sum *= 1000;
  ...
  this.Username = (function () {
    return (
      (string("id('header')//a[contains(@href,'?user=')]/@href").match(
        /([^=]+)$/
      ) || [])[1] || ""
    );
  })();
  ...
  this.Language = (function () {
    return (
      (string("//script[contains(text(),'var nl = ')]/text()").match(
        /var nl = ([""''])?(\w{1,})\1\;$/m
      ) || [])[2] ||
      string("//select[@name='lang']/option[@selected]/@value") ||
      "en"
    );
  })();
  ...
  this.Theme = (function () {
    return (
      ...
      ).match(/\/themes\/([\d\w_]+)/) || [])[1] || "000_def_f65b1"
    );
  })();
  ...
  this.Neofriends = (function () {
    ...
    for (var ai = 0, at = fs.snapshotLength; ai < at; ++ai) {
      ...
      nfs.push({
        Avatar:
          f.parentNode /*div*/.parentNode /*td*/.parentNode /*tr*/
            .cells[0] /*td*/.childNodes[0] /*img*/.src,
        Username:
          f.parentNode /*div*/.previousSibling /*div*/.childNodes[1] /*a*/
            .textContent,
        Online: f.textContent
          .replace(/^\s+|\s+$|\s+\|\s?/g, "")
          .replace("hr", "hour")
          .replace("min", "minute"),
      });
  ...
  this.Debug = function () {
    return [
      ...
      "[ActivePet]\n- " +
        (function (x) {
          with (x) {
            return [Name, Species, Health, Mood, Hunger, Age, Level];
          }
        })(this.ActivePet).join("\n- "),
      "[Neofriends]\n- " +
        (function (x) {
  ...
  if (GM_getValue("DisplayMessage", true)) {
    ...
    if (!(container = document.getElementById("NeopetsMessageContainer"))) {
      ...
      container.setAttribute(
        "style",
        "z-index:11; text-align:right; position:fixed; bottom:0px; right:0px; background-color:#000; font-weight:bold; color:#FFF; display:block; padding:4px; padding-left:7px; min-width:0px; min-height:14px;"
      );
    ...
    function fadeOutElement(elem, ini, min, max, is_parent) {
      ...
      if (!is_parent) {
        ...
        var minWidth = parseInt(
          elem.parentNode.style.minWidth.match(/^\d+/)[0]
        );
      ...
      var t3 = setTimeout(
        function (elem, min, max, is_parent) {
          var t1 = setInterval(
            function (elem, min, max) {
