...
// @version        2.0.5.6
// @language       en
// @include        https://www.neopets.com/games/play.phtml?game_id=*
// @grant          GM_log
// @grant          GM_addStyle
// @grant          GM_getValue
// @grant          GM_setValue
...
// @icon           https://gm.wesley.eti.br/icon.php?desc=66139
...
(function () {
  ...
  if (id in games && cat) {
    ...
    if (games[id][4] === null)
      games[id][4] =
        "https://images.neopets.com/games/clicktoplay/tm_" + id + ".gif";
    else if (!games[id][4]) games[id][4] = "default.gif";
    if (!/^\w+:\//.test(games[id][4]))
      games[id][4] =
        "https://images.neopets.com/neoboards/avatars/" + games[id][4];
    if (games[id][5] === null)
      games[id][5] = xpath(
        "string(id('gamesRoomContentWrap')//div[@class='rcModuleHeaderContent']/div[last()])"
      );
    ava.innerHTML =
      "<a class='info' href='javascript:void(0);'><img width='50' border='0' height='50' src='" +
      games[id][4] +
      "' />" +
      (games[id][5]
        ? "<span class='cat_tooltip'>" + games[id][5] + "</span>"
        : "") +
      "</a>";
    ...
    xpath(".//a/img", ava)[0].addEventListener(
      "error",
      function (e) {
        e.target.src =
          "https://images.neopets.com/neoboards/avatars/default.gif";
      },
      false
    );
    xpath(".//a", ava)[0].addEventListener(
      "click",
      function (e) {
        var t,
          game = games[id],
          ...
          s =
            parseInt(
              prompt(
                I18n.get("npafg.score.prompt") + " " + game[0] + "+)",
                s - (s % game[2])
              ),
              10
            ) || 0;
        ...
        if (
          ...
          (t =
            parseInt(
              prompt(
                I18n.get("npafg.time.prompt"),
                Math.floor(s * game[3] * (1 + 0.2 * Math.random()))
              ),
              10
            ) || 0) > 0
        )
          FlashGame.execute({
              ...
              (function (span, timer, p) {
                ...
                if (
                  ...
                  if (i < 1000) {
                    ...
                    if (
                      /^(?:20|10|3|2|1)$/.test(Math.floor(i / 1000)) &&
                      p.beep.play
                    ) {
                      p.beep.play();
                    }
                ...
                p.params.span[1],
                new Timer({ target: new Date().valueOf() + p.t }),
                p
              );

              document.body.setAttribute(
                "onbeforeunload",
                "return '" + I18n.get("npafg.running.bkgd") + "'"
              );
            },
            onsuccess: function (p) {
              ...
              if (p.list.errcode in msgs) {
                ...
                p.params.span[1].textContent =
                  I18n.get("npafg.error.unknown") + " " + p.list.errcode;
              }
    ...
    FlashGame.menu(
      "cached_includes",
      "[Neopets : Avatars Flash Games] " + I18n.get("inpfg.cache_encrypt.label")
    );
  }
})();
