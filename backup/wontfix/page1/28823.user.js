...
// @icon           https://gm.wesley.eti.br/icon.php?desc=28823
// @noframes
// ==/UserScript==
...
(function () {
  ...
  var script = {
    ...
    language:
      document.evaluate(
        '//select[@name="lang"]/option[@selected]/@value',
        document,
        null,
  ...
  var externalData = evalValue(
    GM_getValue("externalData", '{"response":{"titles":[]}}')
  );
  if (language != script.language || externalData.response.titles.length < 3) {
    resourceText(
      "https://neopets.wesley.eti.br/RandomEvent/getExternalData.php?type=json&language=" +
        script.language,
      function (r) {
  ...
  function logEvent(r) {
    ...
    var events = document.evaluate(
      xpath(
        '//table[tbody/tr/td/*[upper-case(text()) = upper-case("' +
          r.titles.join('") or upper-case(text()) = upper-case("') +
          '")] and tbody/tr/td/img[contains(@src,"https://images.neopets.com/")]]'
      ),
      document,
      null,
    ...
    for (var i = 0, t = events.snapshotLength; i < t; ++i) {
      ...
      if (image && text) {
        queueEvents = queueEvents.jsonGambi(
          {
            date: script.datetime,
            title: table.rows[0].cells[0].firstChild.innerHTML,
    ...
    if (queueEvents != "[]") {
      ...
      resourceText(
        "https://neopets.wesley.eti.br/RandomEvent/addEvent.php?type=json",
        function (r) {
          r = evalValue(r).response;
          if (r.actions) {
            var actions = {
              localUsernameMissing: function () {
                ...
                while (
                  (x = prompt(
                    "Type in your username:\nUsername must have between 5 and 20 caracters",
                    x
                  ))
                ) {
              ...
              localPasswordMissing: function () {
                ...
                while (
                  (x = prompt(
                    "Type in your password: (The password will be shown literally)\nPasswords must have between 5 and 20 OR exactly 32 caracters (MD5 coding)",
                    ""
                  ))
                ) {
                  if ((x.length >= 5 && x.length <= 20) || x.length == 32)
                    break;
                }
...
function cookieValue(p) {
  ...
  return li >= 1 + p.length
    ? cookies.substring(li, cookies.indexOf(";", li))
    : null;
}
    ...
    var options = {
      ...
      onload: function (e) {
        ...
        if (url.match("[?&]type=json")) {
          ...
          if (
            rjson.location &&
            (!rjson.location[0] || !GM_getValue(rjson.location[0], false))
          ) {
...
function xpath(str) {
  ...
  var result = str.match(
    /(?:upper|lower)-case\((['"])?[a-z0-9áàâäãéèêëíìîïóòôöõúùûü !()]+\1(?:,(['"])?[a-z0-9áàâäãéèêëíìîïóòôöõúùûü !()]+\2)*\)/gi
  );
  for (var i = 0, t = result.length; i < t; ++i) {
    var f = result[i].match(
      /([a-z-]+)\(((['"])?[a-z0-9áàâäãéèêëíìîïóòôöõúùûü !()]+\3(?:,(['"])?[a-z0-9áàâäãéèêëíìîïóòôöõúùûü !()]+\4)*)\)/i
    );
...
function vsprintf(format, data) {
  // simulates a simplest version of vsprintf php function
  data.unshift("");
  ...
  for (var i = 0, t = f.length; i < t; ++i) {
    ...
    output +=
      format.substring(ini, format.indexOf(f[i], ini) - 1) +
      pad.substr(0, parseInt(r[3]) - data[r[1]].length) +
      data[r[1]];
    ini += 1 + f[i].length;
  }
...
function getNeopianTime(c) {
  ...
  currentDate.setMinutes(
    currentDate.getMinutes() + currentDate.getTimezoneOffset() - 7 * 60
  );
  if (c) {
    currentDate.setHours(
      parseInt(c[2]) > 12
        ? c[2]
        : 12 * ((c[1] || c[5]).replace(" ", "") == "pm") +
            (parseInt(c[2]) % 12),
      c[3],
      c[4]
    );
    ...
    if (currentDate < ini || currentDate > fim) {
      // daylight saving time
      currentDate.setHours(currentDate.getHours() - 1);
    }
  }
  var output = vsprintf(
    "%4$04d-%9$02d-%3$02d %5$02d:%6$02d:%7$02d",
    (
      currentDate
        .toString()
        .replace(/,| \(.+\)/g, "")
...
function privateStringJsonGambi(data, node, separator) {
  ...
  return (
    this.substring(0, this.length - node) +
    separator +
    data +
    this.substr(this.length - node)
  );
}
/* ###[ /includes ]### */
