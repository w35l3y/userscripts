...
// @icon           https://gm.wesley.eti.br/icon.php?desc=35445
...
// @contributor    jerone ( https://userscripts-mirror.org/topics/19030?page=1#posts-481988 )
// ==/UserScript==
...
(function () {
  ...
  var script = {
      ...
      config: JSON.parse(
        GM_getValue(
          ...
          JSON.stringify({
            ...
            download:
              "[<yyyy><mm><dd>-<hh><nn>] <name> [<version|uso:version>].user.js",
    ...
    toInt = function (x) {
      return parseInt(
        /\d+/.test(xpath("string(" + x + ")").replace(/[.,]/g, ""))
          ? RegExp.lastMatch
          : "",
        10
      );
    ...
    user = xpath(
      "string(id('details')//a[@user_id and contains(@href, '/users/')]/text())"
    ),
  ...
  if (/\/review\//.test(location.href)) {
    var execute = function () {
        var nver = 1 + (toInt("id('content')/p/a/text()") || 0), // number of versions
          fans = toInt(
            "id('script-nav')/li/a[contains(@href, 'fans')]/span/text()"
          ), // fans
          comm = toInt(
            "id('script-nav')/li/a[contains(@href, 'discuss')]/span/text()"
          ), // comments
          inst = toInt("id('details')/text()[contains(., 'nstall')]"), // installs
          revws = toInt(
            "id('script-nav')/li/a[contains(@href, 'reviews')]/span/text()"
          ); // reviews

        if (!(script.current in script.stats)) {
          // current script is new
          stats = script.stats[script.current] = [nver, {}];
        } else if (script.stats[script.current][0] == nver - 1 && isYours) {
          // current script was updated
          stats[0] = nver--;
        } else {
          stats[0] = nver;
        }
    ...
    if (
      isYours ||
      script.current in script.stats ||
      script.config.auto & 1 ||
      listContains(user)
    ) {
      execute();
    }
              ...
              if (a & 4) {
                ...
                if (a & 1) {
                  ...
                  alert(
                    "[" +
                      GM_info.script.name +
                      "]\n\nActivated for the current author! ( " +
                      user +
                      " )"
                  );
                } else if (a & 2) {
                  ...
                  alert(
                    "[" +
                      GM_info.script.name +
                      "]\n\nActivated for the current script! ( #" +
                      script.current +
                      " )"
                  );

                  location.assign(
                    location.href.replace("/review/", "/versions/")
                  );
                }
              }
            }
    ...
    var list = xpath("id('content')/ul[1]/li/a[1]"),
      page =
        (/page=(\d+)/.test(location.search) && parseInt(RegExp.$1, 10)) || 1,
      ...
      smeta = function (obj, e) {
        ...
        e.parentNode.insertBefore(
          document.createTextNode("[ " + x.join(" | ") + " ] "),
        ...
        if (l) {
          GM_xmlhttpRequest({
            ...
            onload: function (xhr) {
              ...
              meta[index].download = script.config.download.replace(
                /<([\w:]+(?:\|[\w:]+)*)>/g,
                function ($0, $1) {
                  var keys = $1.split("|");
                  ...
                  for (var ai in keys) {
                    ...
                    if (
                      /^(?:y{1,2}|y{4}|m{1,2}|d{1,2}|h{1,2}|n{1,2}|s{1,2})$/i.test(
                        key
                      )
                    ) {
                      ...
                      if (!isNaN(d)) {
                        switch (key[0]) {
                          ...
                          case "y":
                            return l > 1
                              ? d.getFullYear().toString().substr(-l)
                              : d.getYear();
                          case "M":
      ...
      if (ii in stats[1]) {
        ...
        if (ss.length) {
          e.parentNode.appendChild(
            document.createTextNode(" [ " + ss.join(" | ") + " ]")
          );

          if (stats[0] == init - i && ss2.length) {
            e.parentNode.appendChild(
              document.createTextNode(" - [ " + ss2.join(" | ") + " ]")
            );
          }
        }
      }
