...
// @icon           https://gm.wesley.eti.br/icon.php?desc=180525
...
(function () {
  // script scope
  var h = xpath(
    ".//thead/tr/th[@class = 'cabecalhoPrincipal' and contains(text(), 'NIT')]"
  )[0];
  if (h) {
    var keyChanged = false,
                ...
                fields: [
                  {
                    ...
                    description:
                      "Link ou chave da planilha no Google Spreadsheet contendo a lista de registros reservados para os testes automatizados.<br /><br />Exemplo: https://docs.google.com/spreadsheet/ccc?key=<b>0ArJZsw7BPFJNdEVYbTFqY2dhc0Z4NkNWLXUyalBTbnc</b>",
                    events: {
                      change: function () {
                        keyChanged = true;
                      },
                  ...
                  },
                  {
                    ...
                    description:
                      "Intervalo de checagem da atualização da planilha",
                    empty: "86400000",
                    value: [
      ...
      opts = win.get("group"),
      cachedNits = JSON.parse(
        GM_getValue("registros", '["2013-10-23T14:12:55.770Z", []]')
      );

    function processaListaNits(lista) {
      xpath(
        ".//tr/td[" + (1 + h.cellIndex) + "]/text()",
        h.parentNode.parentNode.parentNode
    ...
    function forceReset(opts, cb) {
      GM_xmlhttpRequest({
        method: "get",
        url:
          "https://spreadsheets.google.com/tq?tqx=responseHandler:myHandlerFunction&tq=select+A&key=" +
          opts.sheet.key,
        onload: function (xhr) {
    ...
    if (
      !opts.sheet.cache ||
      (cachedNits[1].length &&
        opts.sheet.cache > new Date() - new Date(cachedNits[0]))
    ) {
