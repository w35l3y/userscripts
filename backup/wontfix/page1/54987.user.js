...
// @icon           https://gm.wesley.eti.br/icon.php?desc=54987
...
WinConfig.init = function (opts) {
  return new (function (opts) {
    ...
    this.LoadValues =
      typeof opts.load_values == "undefined" ? true : opts.load_values;
    this.Title =
      opts.title ||
      (function (type) {
        switch (type) {
      ...
      this.Add = function (name, opts) {
        this.List[name] = new (function Button(opts) {
          ...
          this.Event = new (function Events(opts) {
            ...
            this.Add = function (name, opts) {
              if (!(name in this.List)) this.List[name] = [];
            ...
            for (var name in opts) this.Add(name, opts[name]);
          })(opts.events);
        })(opts);
      };
      this.Remove = function (name) {
        if (name in this.List) delete this.List[name];
      };
        ...
        case "prompt":
          return {
            default: {
              fields: {
                text: {
                  value: (load && GM_getValue(prefix + "-text", "")) || "",
                },
              },
            },
          };
          ...
          this.Field = new (function Fields(opts, prefix, load) {
            ...
            this.Add = function (name, opts) {
              this.List[name] = new (function Field(opts, prefix, load) {
                ...
                this.Help = opts.help || "";
                this.isHidden =
                  typeof opts.is_hidden == "undefined" ? false : opts.is_hidden;
                ...
                switch (this.Type) {
                  ...
                  case "string":
                    this.isMulti =
                      typeof opts.is_multi == "undefined"
                        ? false
                        : opts.is_multi;
                    break;
                  case "array":
                    this.isSelect =
                      typeof opts.is_select == "undefined"
                        ? true
                        : opts.is_select;
                    this.isMulti =
                      typeof opts.is_multi == "undefined"
                ...
                if (this.Type == "array" && this.isMulti)
                  this.Value =
                    opts.value ||
                    (load &&
                      JSON.parse(GM_getValue(prefix + "-" + name, "[]"))) ||
                    opts.default ||
                    [];
                else
                  this.Value =
                    opts.value ||
                    (load && GM_getValue(prefix + "-" + name, "")) ||
                    opts.default ||
                    "";
              })(opts, prefix, load);
            };
      ...
      for (var session_name in this.Session.List) {
        ...
        for (var field_name in session_obj.Field.List) {
              ...
              (function (field) {
                return function () {
                  WinConfig.init({
                    ...
                    size: ["450px", 0],
                    title: field.Label,
                    description: "<pre>" + field.Help + "</pre>",
                  })
                    .Open()
                    .FadeIn(0);
                };
          ...
          field.className =
            field_name +
            " field " +
            ((ac && ac.length && ac[alt_classes++ % ac.length]) || "");
          var input;
          switch (field_obj.Type) {
            ...
            case "array":
              if (field_obj.isSelect) {
                ...
                input.name =
                  field_name +
                  (field_obj.isMulti && !field_obj.isSelect ? "[]" : "");
                for (var name in field_obj.List) {
    ...
    this.Save = function () {
      for (var session_name in this.Session.List) {
        ...
        for (var field_name in session_obj.Field.List) {
          var field_obj = session_obj.Field.List[field_name];
          var obj = this.Rendered.elements.namedItem(
            field_name +
          ...
          switch (field_obj.Type) {
            case "int":
              ...
              if (isNaN(sv)) {
                ...
                WinConfig.init({ type: "error", description: "<br />" + msg })
                  .Open()
                  .FadeIn(0);
                throw msg;
              }
              ...
              if (field_obj.isMulti) {
                if (field_obj.isSelect)
                  ...
                  for (; obj; obj = obj.nextSibling) {
                    if (obj.checked) sv.push(obj.value);
                  }
                }
    ...
    this.FadeIn = function (time, interval) {
      if (typeof time == "undefined") time = 900;
      else if (!time) time = 1;
