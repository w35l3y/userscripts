...
// @icon           https://gm.wesley.eti.br/icon.php?desc=38981
// @resource       highlight https://shjs.sourceforge.net/sh_style.css?v1
// @require        https://shjs.sourceforge.net/sh_main.min.js?v1
// @require        https://shjs.sourceforge.net/lang/sh_php.min.js?v1
...
*/
  var replies = document.evaluate(
    "id('mboxfull')/table/tbody/tr[2]/td[1]/div[position()<last()]/div[2]|id('mboxfull')/table[2]/tbody/tr[2]/td[1]/form[1]/div/div|id('mbox')/table[3]/tbody/tr[2]/td[1]/div/div|id('app_content_testimonials')/div/div[2]|id('f')/table/tbody/tr[2]/td[1]/div[5]/div/div[6]|id('mboxfull')/table/tbody/tr[2]/td[1]/div/div",
    document,
    null,
  ...
  for (
    var reply, i = replies.snapshotLength;
    ...
    for (var ai = 0, at = lines.length; ai < at; ++ai) {
        ...
        var bi =
          x[0] > x[1]
            ...
            : Math.max(
                0,
                tabs[0] -
                  (x[0] > 1 && /^}/.test(lines[ai]) && x[0] == x[1] ? 1 : 0)
              );
        bi;
        --bi
      )
      ...
      if (tabs[1] != tabs[0]) tabs[1] = tabs[0];
    }

    reply.innerHTML = output
      .replace(/\t+/gm, "")
      .replace(
        /(?!\[{2})\[code\](?:(?:<br(?:\s*\/)?>)+)?(|[^]+?)((?:<br(?:\s*\/)?>)+)?\[\/code\]/gi,
        '<fieldset style="border:1px #000000 solid;"><legend>[&nbsp;Code&nbsp;]</legend>$1</fieldset>'
      )
      .replace(
        /(?:&lt;?|<)\?((?:php|=|(?!(?:xml|>|&gt;?)))(?:[^]*?(?:<br(?:\s*\/)?>)?)?)((?:|[^]+?))(<br(?:\s*\/)?>)?\?(?:&gt;?|>)/gi,
        '<pre class="sh_php" style="display:inline; white-space:pre-wrap;">&lt;?$1<span style="background-color:inherit; display:inline-block;">$2</span>$3?&gt;</pre>'
      )
