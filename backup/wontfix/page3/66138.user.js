...
// @icon           https://gm.wesley.eti.br/icon.php?desc=66138
...
/**************************************************************************
    ...
    Based on https://userscripts-mirror.org/scripts/version/63649/158447.user.js?format=txt (by Backslash)
  ...
  switch (type) {
    case "play":
      var msg = xpath(".//div[@class = 'errormess' and b]", doc)[0],
        ...
        type = xpath(
          "id('gamesRoomContentWrap')//td[1]//div//td[2]//area[1]",
          doc
        )[0];
      ...
      if (type) {
        obj = {
          ...
          message: msg,
          id: parseInt(
            /\bgame_id=(\d+)/.test(type.getAttribute("onclick")) && RegExp.$1,
            10
          ),
          name: xpath(
            "string(id('gamesRoomContentWrap')//td[1]//div[4]/div[3])",
            doc
          ),
          ratio: parseFloat(
            xpath(
              "string(id('gamesRoomContentWrap')//td[1]/div/div//tr[2]/td/a[1])",
              doc
            ).match(/\d+(?:[.,]\d+)?/)
          ),
        ...
        obj = {
          ...
          id: parseInt(
            /\bgame_id=(\d+)/.test(
              xpath(
                "string(.//script[contains(text(), 'game_id')]/text())",
                doc
              )
            ) && RegExp.$1,
            10
    ...
    case "play_flash":
      ...
      xpath("id('game-info')//td[1]//tr[2]//strong/div", doc).forEach(function (
        h
      ) {
        obj.highscores.push({
          ...
          score:
            parseInt(
              ("" + h.textContent.match(/\d+(?:[,.]\d+)+/)).replace(
                /[,.]/g,
                ""
              ),
              10
      ...
      var flashvars = xpath(
          "string(id('gameWrapper')//param[@name='flashvars']/@value|id('gameWrapper')//script[contains(., 'gamePreloader')]/text())",
          doc
        ),
        p;
    ...
    case "process_flash_score":
      var obj = { list: {} };
      String(typeof doc == "string" ? doc : doc.textContent)
        .split("&")
        .map(function (a) {
          var o = a.split("=", 2);
...
FlashGame.open = function (params) {
  if (!("referer" in params)) {
    try {
      params.referer =
        "https://www.neopets.com/games/play.phtml?game_id=" +
        ((/game_id=(\d+)/.test(params.url) && RegExp.$1) || params.id);
    } catch (e) {}
  }

  if (!/^http:\/\/www\.neopets\.com\/games\/play_flash\.phtml/.test(params.url))
    alert(
      "[Includes : Neopets : FlashGame : open]\n" +
...
FlashGame.url = function (params) {
  function x(params) {
    ...
    if (
      (i && i.Decimals in decimals_arr && decimals_arr[i.Decimals].length) ||
      params.score <= 0 ||
      params.time <= 0
    ) {
      var encode = {
      ...
      var o = "",
        ...
        t = {
          ...
          gmdt_g: (function (c, x) {
            ...
            for (var ai = 1, at = str.length, k = 0; ai < at; ++ai) {
              ...
              output += (
                "000" +
                String(
                  ~x ? hex[(x + hex.indexOf(bin[k])) % hl] : str[ai]
                ).charCodeAt(0)
              ).substr(-3);
              k = ++k % bl;
            }
            ...
            return (
              output +
              ("000" + rnd[0].charCodeAt(0)).substr(-3) +
              ("000" + rnd[1].charCodeAt(0)).substr(-3)
            );
          ...
          fs_g:
            params.opts.forceScore != undefined
              ? params.opts.forceScore || ""
              : idv.fs_g,
        };
      ...
      return (
        "https://www.neopets.com/high_scores/process_flash_score.phtml?" +
        o.substr(1)
      );
    } else return false;
  }
  ...
  if (
    params.opts.include_movie &&
    /([\w_]+)\.swf$/.test("" + params.opts.include_movie)
  ) {
  ...
  if (params.include && params.include in FlashGame.cached_includes) {
    ...
    if (
      FlashGame.cached_includes[params.include].Decimals in decimals_arr &&
      !(params.include in FlashGame.includes) &&
      (ci == 2 ||
        (ci == 1 &&
          confirm(
            "[Includes : Neopets : FlashGame]\n\n" +
              I18n.get("inpfg.cached_encrypt.confirm", [
                FlashGame.cached_includes[params.include].LastUpdate ||
                  I18n.get("unknown"),
              ])
          )))
    ) {
      FlashGame.includes[params.include] =
        FlashGame.cached_includes[params.include];
    }
  }

  var alertMsg =
    "[Includes : Neopets : FlashGame : url]\n" +
  ...
  if (nan.length)
  ...
  else if (!params.include) alert(alertMsg.replace("{0}", "include"));
  else if (!params.game) alert(alertMsg.replace("{0}", "game"));
  else if (!params.time) alert(alertMsg.replace("{0}", "time"));
  else if (!params.score) alert(alertMsg.replace("{0}", "score"));
  ...
  else if (!params.onsuccess) alert(alertMsg.replace("{0}", "onsuccess"));
    ...
    ShowMyCode.execute({
      ...
      onsuccess: function (_params) {
        ...
        var ini =
            /^\s+(?:public )?class (?:np\.projects\.(?:np\d+\.classCrypt|include\.Strings)|NP9_Score_Encryption {)/im,
          include = {
        ...
        if (ini.test(_params.response.text)) {
          ...
          if (
            /functiongetiVID\(\)(?::number)?{return\((\d{5,})\);}/i.test(
              content.replace(/\s+/g, "")
            )
          ) {
            ...
            while (re.exec(content) != null) {
              decimals.push(
                RegExp.$1.split(",").map(function ($0) {
                  return parseInt($0, 10);
                })
              );
            }

            include.Decimals = MD5_hexhash(
              decimals.toSource().replace(/\s+/g, "")
            );
          }
        ...
        if (
          is_error &&
          obj.include in FlashGame.cached_includes &&
          GM_getValue("cached_includes", 1) == 0 &&
          confirm(
            "[Includes : Neopets : FlashGame]\n\n" +
              I18n.get("inpfg.cached_encrypt.confirm")
          )
        ...
        obj.error = is_error
          ? 2
          : /^(?:13960|89198|97250)$/.test(include.Vid)
          ? 0
          : 1;
        ...
        if (!obj.error) {
          ...
          var decimals_arr = JSON.parse(GM_getValue("decimals", "{}"));
...
FlashGame.send = function (params) {
  ...
  if (
    !/^http:\/\/www\.neopets\.com\/high_scores\/process_flash_score\.phtml/.test(
      params.url
    )
  )
    alert(
      "[Includes : Neopets : FlashGame : send]\n" +
        I18n.get("inpfg.parameter.missing", ["url"])
    );
  else if (!params.referer)
    alert(
    ...
    HttpRequest.open({
      ...
      onsuccess: function (_params) {
        var obj = _params,
          result =
            FlashGame.convert(_params.response.text, "process_flash_score") ||
            {};
        for (var v in params) {
          obj[v] = params[v];
        }
        ...
        if (obj.list && obj.list.call_url) {
          // example : https://www.neopets.com/games/display_avatar.phtml?id=130
        ...
        try {
          var data =
            JSON.parse(
              decodeURIComponent(obj.list.call_external_params).substr(5)
            ) || [];
            ...
            switch (d.fn) {
              ...
              case "flash_func_trigger":
                if (
                  d.args &&
                  d.args.func &&
                  typeof window[d.args.func] == "function"
                ) {
...
FlashGame.execute = function (params) {
  FlashGame.open({
    url: "https://www.neopets.com/games/play_flash.phtml?game_id=" + params.id,
    onsuccess: function (_params) {
      (params.recursive = function (p) {
        ...
        if (p.ratio_score) p.s += Math.floor(1000 / p.list.n);
        p.s -= p.s % p.score[2];
        if ("max_score" in p && p.s > p.max_score) {
          p.s =
            p.max_score -
            Math.floor(p.score[1] * p.score[2] * Math.min(Math.random(), 0.5));
          p.s -= p.s % p.score[2];
        }
        p.t =
          p.time[0] * (p.ratio_time ? p.s : 1) +
          Math.floor(p.time[1] * Math.random());
        ...
        FlashGame.url({
          ...
          onsuccess: function (params) {
            ...
            if (params.output) {
              function ms2str(v) {
                return (
                  ("0" + Math.floor(v / 60000)).substr(-2) +
                  ":" +
                  ("0" + Math.floor((v % 60000) / 1000)).substr(-2)
                );
              }
              ...
              for (var label in n) {
                o.push(
                  [I18n.get("inpfg." + label + ".label"), n[label]].join("\t\t")
                );
              }
              ...
              if (!params.beep) {
                ...
                params.beep.setAttribute(
                  "src",
                  "https://www.soundjay.com/button/beep-8.wav"
                );
                ...
                if (params.session === true)
                  params.session = setInterval(function () {
                    HttpRequest.open({
                      ...
                      url: "https://www.neopets.com/games/session_keep_alive.phtml",
                    }).send();
                  }, 900000);
...
FlashGame.test = function (querystring, crypt) {
  ...
  var hex = String.fromCharCode.apply(
      this,
      crypt[0] instanceof Array ? crypt[parseInt(str.substr(-2), 10)] : crypt
    ),
    hl = hex.length,
    bin = qs.sh_g + qs.sk_g,
    bl = bin.length,
...
FlashGame.menu = function (type, value) {
  var menus = {
    cached_includes: function () {
      ...
      alert(
        "[Includes : Neopets : FlashGame]\n\n" +
          I18n.get("inpfg.cache_status.alert") +
          " " +
          I18n.get(
            "inpfg." + (ci == 2 ? "activated" : "deactivated") + ".label"
          ).toUpperCase()
      );
    },
  };
  ...
  if (type in menus) {
    GM_registerMenuCommand(
      value ||
        "[Includes : Neopets : FlashGame] " +
          I18n.get("inpfg.cache_encrypt.label"),
      menus[type]
    );
  }
};
