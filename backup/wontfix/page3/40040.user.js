...
// @icon           https://gm.wesley.eti.br/icon.php?desc=40040
...
(function () {
  ...
  var topics = document.evaluate(
    "id('content')/table/tbody/tr[td[2]/strong]",
    document,
    null,
  ...
  if (topics.snapshotLength) {
    function displayTopics(topics, link) {
      var isHidden =
        (!link.textContent && GM_getValue("isHidden", true)) ||
        /^Un/.test(link.textContent);
      link.textContent = (isHidden ? "H" : "Unh") + "ide topics"; // Hide/Unhide topics

      for (
        var topic, i = topics.snapshotLength;
        (topic = topics.snapshotItem(--i));

      ) {
        topic.style.backgroundColor = "#F6F6F6";

        if (/grey/.test(topic.cells[0].childNodes[1].getAttribute("class")))
          // no recent activity
