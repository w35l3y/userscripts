...
// @include        https://userscripts-mirror.org/forums/*
// @include        https://userscripts-mirror.org/forums
// @include        https://userscripts-mirror.org/scripts/discuss/*
// @grant          GM_log
// @grant          GM_addStyle
// @grant          GM_getValue
// @grant          GM_setValue
...
// @icon           https://gm.wesley.eti.br/icon.php?desc=39024
...
(function () {
    ...
    if (tries == 3) {
      ...
      GM_xmlhttpRequest({
        ...
        onload: function (e) {
          if (/^2/.test(e.status)) {
            if (
              /\/forums\/\d+/.test(e.finalUrl) &&
              /\/forums$/.test(location.href)
            ) {
              var text = e.responseText.replace(
                /\s+|<\/td>|<td class="c2">|Sticky: <strong>|, this topic is locked\./gi,
                ""
              );

              var total = (
                text.match(
                  /title="Recentactivity"\/><ahref="\/topics\/\d+"/gi
                ) || []
              ).length;
              ...
              if (!(span = document.getElementById(e.finalUrl))) {
                ...
                posts[0][0].parentNode.insertBefore(
                  span,
                  posts[0][0].nextSibling
                );
              }
              ...
              for (
                var m, i = 1;
                (m = /title="Recentactivity"\/><ahref="(\/topics\/\d+)"/gi.exec(
                  text
                ));
                ++i
              ) {
                var a = document.createElement("a");
                a.setAttribute("href", "https://userscripts-mirror.org" + m[1]);
                posts.push([
                  a,
                  function (e, s, i, t) {
                    s.textContent = " (" + i + "/" + t + ")";
  ...
  if (/\/topics\/(?:new\?forum_id=)?\d+/.test(location.href)) {
    var forum = document.evaluate(
      "id('content')/div/a[contains(@href,'/forums/')][1]",
      document,
      null,
    ...
    if (forum) {
      document
        .evaluate(
          "id('new_topic') | id('reply')//form[1]",
          document,
          null,
          XPathResult.FIRST_ORDERED_NODE_TYPE,
          null
        )
        .singleNodeValue.addEventListener(
          "submit",
          function (e) {
    ...
    var subtitle = document.evaluate(
      "id('content')/p[@class='subtitle']",
      document,
      null,
      XPathResult.FIRST_ORDERED_NODE_TYPE,
      null
    ...
    var posts = document.evaluate(
      "//tr[td/img[contains(@class,'green')]]/td[2]//a",
      document,
      null,
    ...
    if (subtitle && posts.snapshotLength) {
      ...
      amark.addEventListener(
        ...
        function (e) {
          ...
          var update = function (e) {
            ...
            while (!img.cells) img = img.parentNode;

            img = img.cells[0].childNodes[1];
            img.setAttribute(
              "class",
              img.getAttribute("class").replace("green", "grey")
            );
          };
      ...
      pmark.setAttribute(
        "style",
        "float:right; margin-top:0pt; margin-left:10pt;"
      );
