...
// @icon           https://gm.wesley.eti.br/icon.php?desc=35215
...
// @require        https://gm.wesley.eti.br/userscripts/PeopleRanking/peopleranking_default.js?v1
// ==/UserScript==
...
(function () {
  ...
  var user = {
    ...
    ranking: GM_getValue("ranking", "scripts;comments;posts")
      .toLowerCase()
      .split(";"),
  };
  ...
  if (!!qs.sort) {
    ...
    for (var column = 0, t = header.length; column < t; ++column) {
      if (header[column].textContent.toLowerCase() == qs.sort) break;
    }
        ...
        btn.addEventListener(
          ...
          function (event) {
            ...
            xpath("//table/tbody/tr/td[1]/a").forEach(function (elem, index) {
              ...
              else if (uid in ranking) delete ranking[uid];
    ...
    xpath("//table/tbody/tr/td[1]/a").forEach(function (elem, index) {
      ...
      var c = "?";
      var cv = parseInt(
        elem.parentNode.parentNode.cells[column].textContent,
        10
      );
      if (uid in ranking) {
        c = "+0";
        if (!user.position || ranking[uid][1] != cv) {
          ...
          if (user.update & Update.Automatic) {
            if (cp <= user.pages) ranking[uid] = [(cp - 1) * 50 + index, cv];
            else delete ranking[uid];
          }
        }
