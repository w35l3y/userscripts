...
// @namespace      https://gm.wesley.eti.br/includes/orkut
...
// @version        1.0.2.0
// @language       en
// @include        https://www.orkut.tld/*
// @exclude        https://www.orkut.tld/Main#*
// @grant          GM_log
// @grant          GM_addStyle
// @grant          GM_getValue
// @grant          GM_setValue
...
// @icon           https://gm.wesley.eti.br/icon.php?desc=62957
...
Orkut._submit = function (params) {
  ...
  params.url = "https://" + location.host + params.url;
    ...
    onsuccess: function recursive(params) {
      ...
      if (
        ...
        WinConfig.init({
          ...
          type: "prompt",
          description:
            "<img src='/CaptchaImage?xid=" +
            Math.random() +
            "' /><br /><br />" +
            obj.message,
          positiveCallback: function (w, e) {
            params.submit.parameters.cs =
              e.form.elements.namedItem("text").value;
...
Orkut.Community.Member._submit = function (params) {
  if (!params.url) {
    var cmm =
      params.parameters.cmm ||
      params.parameters.commId ||
      (location.search.match(/\bcmm=(\d+)\b/) && RegExp.$1);
    var uid =
      params.parameters.uid ||
      (location.search.match(/\buid=(\d+)\b/) && RegExp.$1);
...
Orkut.Community.Forum._sumbit = function (params) {
  if (!params.url) {
    var cmm =
      ...
      (location.search.match(/\bcmm=(\d+)\b/) && RegExp.$1);
    var tid =
      params.parameters.tid ||
      params.parameters.topicId ||
      (location.search.match(/\btid=(\d+)\b/) && RegExp.$1);

    switch (params.action) {
      case "submit":
        if ("subject" in params) params.parameters.subjectText = params.subject;
        if ("text" in params) params.parameters.bodyText = params.text;
        params.url = "/CommMsgPost?cmm=" + cmm;

        if (tid && !params.parameters.tid) params.url += "&tid=" + tid;
        break;
      case "delete":
...
Orkut.Community.Forum.Topic.delete = function (params) {
  for (var key in params.keys)
    params.parameters["topicKeys_" + (1 + parseInt(key, 10))] =
      params.keys[key];
...
Orkut.Scrapbook._submit = function (params) {
  if (!params.url) {
    ...
    switch (params.action) {
      case "submit":
        if ("text" in params) params.parameters.scrapText = params.text;
        params.url = "/Scrapbook?uid=" + uid;

        if (uid && !params.parameters.uid) params.url += "&uid=" + uid;
        break;
    }
  }
