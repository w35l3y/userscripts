...
// @include        https://userscripts-mirror.org/home/scripts*
// @grant          GM_log
// @grant          GM_addStyle
// @grant          GM_getValue
// @grant          GM_setValue
...
// @icon           https://gm.wesley.eti.br/icon.php?desc=35589
// @require        https://www.wesley.eti.br/includes/js/php.js?v1
// @require        https://www.wesley.eti.br/includes/js/php2js.js?v1
// @require        https://gm.wesley.eti.br/gm_default.js?v1
// ==/UserScript==
...
checkForUpdate({
  //	'file':'https://gm.wesley.eti.br/userscripts/IssuesReport/userscripts__issues_repo.user.js',
...
(function () {
  ...
  var script = {
    ...
    user:
      "" +
      xpath("string(id('homeMenu')/li/a[contains(@href,'users')]/@href)").match(
        /\d+/
      )[0],
    author:
      "" +
      xpath(
        "string(id('details')/span/a[contains(@href,'users')]/@href)"
      ).match(/\d+/),
  ...
  if (/\/scripts\/\w+\//.test(location.href) && script.user == script.author) {
    ...
    retrieve.childNodes[0].addEventListener(
      "click",
      function (e) {
        ...
        function listReport(id) {
          ...
          for (var prop in script.issues[id]) {
            ...
            var out =
              "<p>%TP% script: <a href='/scripts/show/%SID%'>%SNM%</a> written by <a href='/users/" +
              script.user +
              "'>" +
              unm +
              "</a></p><p style=\"clear: both\">Reporter: <a href='/users/%RID%'>%RNM%</a></p>";
            if (!!i[5].length) out += "<pre>%MSG%</pre>";
            issue.innerHTML = out
              .replace("%TP%", i[0])
              .replace("%SID%", i[1])
              .replace("%SNM%", i[2])
        ...
        var total = 0;
        if (!!id) total = listReport(id);
        else for (var prop in script.issues) total += listReport(prop);

        if (!total) {
          var msg = document.createElement("h2");
          msg.innerHTML = !!id
            ? "This script does not have any report."
            : "Your scripts does not have any report.";
    ...
    } else retrieve = retrieve.childNodes[0].childNodes[1];
    ...
    if (script.lastAccess + user.access <= current) {
      ...
      var er_issue = new RegExp(
        "<div><p>(\\w+) script: <a href='/scripts/show/(\\d+)'>(.{1,256})</a>written by <a href='/users/" +
          script.user +
          "'>.{3,50}</a></p>(?:<p>.+?</p>)?<p>Reporter: <a href='/users/(\\d+)'>(.{3,50})</a></p>(?:<pre>(.+?)</pre>)?</div>",
        "gi"
      );
      var gc = 0; // global counter
      var lastPage = 0;

      resourceText("https://userscripts-mirror.org/issues", function (e) {
        lastPage =
          parseInt(
            e.responseText.match(
              /(\d+)<\/a> <a href="\/issues\?page=\d+" class="next_page"/
            )[1],
            10
          ) - script.lastPage;
        checkIssue(e);
        ...
        for (var i = 2; i <= lastPage; ++i) {
          setTimeout(
            resourceText,
            user.delay * i,
            "https://userscripts-mirror.org/issues?page=" + i,
            checkIssue
          );
        }
      });
      ...
      function checkIssue(e) {
        var result = e.responseText.replace(
          /\s{2,}|(?: \w+="[ \w:;%-]+")+/g,
          ""
        );
        for (var m; (m = er_issue.exec(result)); ) {
          ...
          m.shift();
          if (!(m[1] in script.issues)) script.issues[m[1]] = {};
        ...
        if (r == 100) {
          ...
          var x = 0;
          for (var prop in script.issues[id]) ++x;

          retrieve.innerHTML = x;
        } else retrieve.innerHTML = r + "%";
      ...
      int = setInterval(function () {
        ...
        if (r == -1) {
          ...
          var x = 0;
          for (var prop in JSON.parse(GM_getValue("issues", "{}"))[id]) ++x;
    ...
    function totalIssues(tr) {
      var sum = 0;
      for (var prop in script.issues[tr.id.match(/\d+/)[0]]) ++sum;
    ...
    if (/[?&]sort=issues/.test(location.href)) {
      scripts.sort(function (a, b) {
        var ta = totalIssues(a),
          tb = totalIssues(b);

        if (ta == tb) return 0;
        else if (ta < tb) b.parentNode.insertBefore(b, a);
        return ta < tb ? 1 : -1;
      });
    }
