...
// @namespace      https://gm.wesley.eti.br/dicingdangers
// @include        https://92.48.103.52/fantasy/*
// ==/UserScript==
...
(function () {
  ...
  // your Player Id
  var id = GM_getValue("PlayerId");
  if (!id) GM_setValue("PlayerId", prompt("What is your Player ID?"));
    ...
    function (e) {
      if (
          ...
          (e.target &&
            (e.target.nodeName.toLowerCase() == "textarea" ||
              (e.target.nodeName.toLowerCase() == "input" &&
                /^(?:text|file)$/i.test(e.target.type))))
        )
      ) {
        ...
        if (char in Shortcut) {
          if (Shortcut[char])
            top.location.href =
              "https://92.48.103.52/fantasy/game.php?" + Shortcut[char];
        } else alert("Character: " + char);
      }
    },
    false
  );
