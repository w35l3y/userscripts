...
// @version        1.0.0.1
// @language       en
// @include        https://www.neopets.com/medieval/archives.phtml*
// @grant          GM_log
// @icon           https://gm.wesley.eti.br/icon.php?desc=88235
...
(function () {
  ...
  function arraySum(t) {
    var s = 0;
    for (var ai = 0, at = t.length; ai < at; ++ai) s -= -t[ai]; // s += t[ai] (concat)
  ...
  function sortByCategory(c, a, b) {
    ...
    switch (c) {
      ...
      case 2: // geometric shapes -> rainbow colors
        ...
        if (!c) return c1 - c2;
        else return colors[c1] - colors[c2];
      case 3: // book spines -> rainbow colors
        return (
          parseInt(a.getAttribute("class").match(/\d$/), 10) -
          parseInt(b.getAttribute("class").match(/\d$/), 10)
        );
      ...
      case 5: // number backwards
        ...
        if (c == 1) return parseInt(n1, 10) - parseInt(n2, 10);
        else if (c == 4) return arraySum(n1.split("")) - arraySum(n2.split(""));
        else
          return (
            parseInt(n1.split("").reverse().join(""), 10) -
            parseInt(n2.split("").reverse().join(""), 10)
          );
    }
  ...
  for (var key in shelfs) {
    var cat = categories[shelfs[key].textContent],
      books = xpath(
        ".//div[contains(@class, 'book') and div[contains(@class, 'symbol" +
          cat[0] +
          "') and not(substring-after(@class, 'symbol" +
          cat[0] +
          "'))]]"
      ).sort(function (
        a,
        b // contains() and not(substring-after()) -> ends-with()
      ) {
    ...
    for (
      var ai = 15 * key,
        bi = 60 + Math.floor(60 * Math.random()),
        at = ai + books.length;
      ai < at;
      ++ai
    ) {
