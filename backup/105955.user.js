...
// @version        2.1.0
// @language       en
// @include        https://www.neopets.com/altador/colosseum/standings.phtml
// @include        https://www.neopets.com/altador/colosseum/schedule.phtml?day=*&team=all
// @grant          GM_getValue
// @grant          GM_setValue
// @icon           https://gm.wesley.eti.br/icon.php?desc=105955
...
(function () {
  ...
  if ("/altador/colosseum/schedule.phtml" == location.pathname) {
    if (/day=([a-z]+)/.test(location.search) && "all" != RegExp.$1) {
      var day = RegExp.$1,
        list = xpath(
          "id('schedule')/div/table/tbody/tr[td[img[contains(@src, '/vs.')]]]"
        );
      ...
      for (var ai = list.length - 1; ~ai; --ai) {
        var rteams = xpath(
            "./td[img[contains(@src, '/vs.')]]/img[@onclick]",
            list[ai]
          ).map(function ($0) {
            return {
          ...
          rgoals = xpath(
            "./td[.//img[@width = 45]]/text()[position() = last()]",
            list[ai]
          );
    ...
    var totals = {};
    xpath(
      ".//div[@class = 'standingModMiddle']//td/b|.//div[@class = 'standingTrophyDiv']"
    ).forEach(function (team) {
      if (
        /([\w ]+)\s+\((\d+)\)/.test(team.textContent) ||
        /([\w ]+)\s+(\d+)/.test(team.textContent)
      ) {
    ...
    xpath(
      ".//div[@class = 'standingBracketPoints' and b[not(text())]]"
    ).forEach(function (points) {
      var team = points.parentNode.textContent.trim();

      points.firstElementChild.textContent =
        totals[team] -
        xpath(
          "sum(.//div[@class = 'standingBracketPoints' and b[text()] and preceding-sibling::div[@class = 'standingBracketName' and b[text() = '" +
            team +
            "']]]/b/text())"
        );
    });
    ...
    xpath(
      ".//div[@class = 'standingMod' and .//table[@class = 'standingBracketMiddle']]"
    ).forEach(function (roundNode) {
      var roundNumber =
        /round-(\d+)/.test(
          xpath("string(.//div[@class = 'standingModTitle']/@style)", roundNode)
        ) && RegExp.$1 - 1;

      xpath(".//div[@class = 'standingBracketLogo']", roundNode).forEach(
        function (teamNode) {
          var id = /(\w+)_\d/.test(teamNode.getAttribute("style")) && RegExp.$1,
            sup = document.createElement("sup");

          sup.textContent =
            id && sum[rounds[roundNumber]] && sum[rounds[roundNumber]][id]
              ? sum[rounds[roundNumber]][id].reduce(function (
                  previous,
                  current
                ) {
