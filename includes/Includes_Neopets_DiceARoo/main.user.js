...
var DiceARoo = function (page) {
	...
	_post = function (data, cb) {
		var url = "http://www.neopets.com/games/play_dicearoo.phtml";
		...
		page.request({
			...
			callback: function (obj) {
				obj.dice = (/\/dice\/(\D+)(\d+)/.test(xpath("normalize-space(.//td[@class = 'content']//div/center/img[1][not(following-sibling::img[1]) and contains(@src, '/games/dice/')]/@src)", obj.body))?[dices.indexOf(RegExp.$1), parseInt(RegExp.$2, 10)]:[-1, -1]),
				obj.type = (/\/(\w+)\/blumaroo_(\w+)_/.test(xpath("normalize-space(.//td[@class = 'content']//div/center/img[contains(@src, '_baby.gif')]/@src)", obj.body))?stats.indexOf(RegExp.$1 + "_" + RegExp.$2):-1);
	...
	this.jackpot = function (obj) {
		page.request({
			method	: "get",
			action	: "http://www.neopets.com/games/dicearoo.phtml",
			referer	: "http://www.neopets.com/games/dicearoo.phtml",
			delay	: [900, 500],
			callback: function (o) {
				o.dices = [];
				o.prizes = [];
