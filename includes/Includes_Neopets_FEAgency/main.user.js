...
//page.database.truncate("fea");
	if (!page.database.tableExists("fea")) {
		page.database.createTable("fea", ["id", "item_ID", "quantity", "time", "reward", "type"]);
	...
	var _get = function (data, cb) {
		page.request({
			method	: "get",
			action	: "http://www.neopets.com/faerieland/employ/employment.phtml",
			referer	: "http://www.neopets.com/faerieland/employ/employment.phtml?type=jobs",
			delay	: true,
			data	: data,
			callback: function (o) {
	...
	parse = function (data, cb, o) {
		...
		case "jobs":
			o.isNew = false;
			o.jobs = xpath(".//td[@class = 'content']/center/table/tbody/tr/td/b/a[contains(@href, '&job_id=')]", o.body).map(function (item) {
				var row = xpath("./ancestor::tr[1]", item)[0],
				info = xpath("./b[1]/text()|./text()", row.nextElementSibling.cells[0]).map(function (value, index) {
					return (1 == index?value.textContent.trim():parseInt(value.textContent.replace(/\D+/g, ""), 10));
				}),
				output = {
					id		: parseInt(item.href.match(/job_id=(\d+)/)[1], 10),
					type	: (/background-color:(#\w+);/.test(row.cells[0].getAttribute("style"))?RegExp.$1:"#000000").toUpperCase(),
					quantity	: info[0],
					item	: {
						name	: info[1],
		...
		case "apply":
			...
			page.database.commit();
		case "desc":
			var texts = xpath(".//td[@class = 'content']/center/b/text()|.//td[@class = 'content']/center/b/b/text()", o.body),
			finished = !xpath("boolean(.//td[@class = 'content']//a[contains(@href, 'job_quit=')])", o.body),
			n = function (v) {
			...
			if (!o.error && "apply" == data.type && (o.error = finished)) {
				o.errmsg = xpath("normalize-space(.//td[@class = 'content' and not(div)]/text()[normalize-space(.)]|.//td[@class = 'content' and not(div)]/center[2]|.//td[@class = 'content']/div[a[contains(@href, 'type=desc')]])", o.body);
				o.found = !(~o.errmsg.indexOf("job could not be found") || ~o.errmsg.indexOf("No job with this ID was found"));
			}
			try {
	...
	this.allJobs = function (obj) {
		...
		_this.jobs({
			...
			callback: function (o) {
				...
				}(o, (/start=(\d+)/.test(xpath("string(.//td[@class = 'content']/center/b[position() = last()]/a[contains(@href, 'start')]/@href)", o.body))?parseInt(RegExp.$1, 10):0), true));
			}
		});
	};
