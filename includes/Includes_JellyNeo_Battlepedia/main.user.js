...
var Battlepedia = function () {
  var count = function (nodes) {
    ...
    nodes.forEach(function (item) {
      var k = /newicons\/(\w)/.test(item.getAttribute("src"))
          ? RegExp.$1.toUpperCase()
          : undefined,
        type =
          item.nextSibling &&
          /\+|\d+(?=%)|all/i.test(item.nextSibling.textContent)
            ? RegExp["$&"]
            : undefined;
  ...
  this.search = function (obj) {
    ...
    (function recursive(start) {
      ...
      HttpRequest.open({
        method: "get",
        url: "https://battlepedia.jellyneo.net/index.php",
        headers: {
          Referer: "https://battlepedia.jellyneo.net/index.php",
        },
        onsuccess: function (xhr) {
          var num = xpath(
            "string(id('content')/h3/text()[starts-with(., 'Showing results')])",
            xhr.response.xml
          )
            .split(/\D+/g)
            .map(parseFloat);
          ...
          Array.prototype.push.apply(
            output.list,
            xpath("id('content')//tr[td[1]/a]", xhr.response.xml).map(function (
              row
            ) {
              var img = xpath("./td[1]//img", row)[0];
              return {
                id: /id=(\d+)/.test(img.parentNode.getAttribute("href"))
                  ? parseInt(RegExp.$1, 10)
                  : undefined,
                jnId: /showitem=(\d+)/.test(
                  xpath(
                    "string(.//a[img[contains(@src, 'icon_link_itemdatabase')]]/@href)",
                    row
                  )
                )
                  ? parseInt(RegExp.$1, 10)
                ...
                image: /items\/(\w+)/.test(img.getAttribute("src"))
                  ? RegExp.$1
                  : undefined,
                ...
                rating: parseFloat(
                  xpath(
                    "string(.//div[contains(@class, 'rating')]/text())",
                    row
                  )
                ),
              };
