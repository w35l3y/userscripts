...
// @contributor    minicoz (https://userscripts.org/topics/125966#posts-498972)
...
Wizard.convert = function (params) {
  ...
  switch (params.type) {
    case 1: // is_super (1)
      ...
      for (var ai = 0, at = res.data.rowcount; ai < at; ++ai) {
        var owner = res.data.owners[ai],
          price = parseInt(
            (res.data.prices[ai] || res.data.prices_str[ai])
              .replace(/[,.]+/g, "")
              .match(/^\d+/),
            10
          );
        ...
        r.push({
          Id: id,
          Link: [
            "https://www.neopets.com/browseshop.phtml?owner=" + owner,
            "buy_obj_info_id=" + id,
    ...
    default: // others (0, undefined)
      try {
        var items = xpath(
            ".//td[@class = 'content']//table[not(.//img)]//tr[position() > 1]",
            params.doc
          ),
          msg = xpath(
            ".//div[contains(@class, 'errormess')]/div[b] | .//td[@class = 'content']//center/b | .//td[@class = 'content']/div[2]/div[2]/b",
            params.doc
          )[0];
      } catch (e) {
        var info = GM_info,
      ...
      return {
        search: xpath(
          "string(.//td[@class = 'content']//table[1]//td[2]/b/span/text())",
          params.doc
        ),
        list: r,
    ...
    var _pw = params.wait || [500, 500],
      process = function (p1, p2, r) {
        ...
        if (r && !p1.error && 0 < --p1.attempts) {
          ...
          p1[
            p1.error && typeof p1.onerror == "function"
              ? "onerror"
              : "onsuccess"
          ](p1);
        }
    ...
    if (params.is_super) {
      (function recursive(params) {
        HttpRequest.open({
          ...
          priceOnly: "0",
          context:
            typeof params.is_shop == "undefined" || params.is_shop ? "0" : "1", // 0=shop 1=gallery
          partial:
            typeof params.is_exact == "undefined" || params.is_exact
              ? "0"
              : "1",
          min_price:
            parseInt(
              ("" + params.min_price).replace(/[,.]+/g, "").substr(0, 6),
              10
            ) || 0,
          max_price:
            parseInt(
              ("" + params.max_price).replace(/[,.]+/g, "").substr(0, 6),
              10
            ) || 999999,
          json: "1",
        });
      ...
      (function recursive(params) {
        HttpRequest.open({
          ...
          table:
            typeof params.is_shop == "undefined" || params.is_shop
              ? "shop"
              : "gallery",
          criteria:
            typeof params.is_exact == "undefined" || params.is_exact
