...
(function (parent) {
  var fixedTemplate = function (xhr, findArena, findPirate) {
    ...
    return {
      list: xpath(
        ".//tr[2 < position() and td[5] and not(ancestor::table[1]/tbody/tr[position() = last()]/td[3])]",
        xhr
      ).map(function (bet) {
        ...
        return {
          ...
          arenas: xpath("./b", bet.cells[1]).map(function (arena) {
            ...
            _an.pirate = findPirate(
              arena.nextSibling.textContent.trim().slice(1).trim()
            );
  ...
  parent.FoodClub = function (page) {
    var _this = this,
      ...
      _get = function (data, p, cb) {
        page.request({
          method: "get",
          action: "https://www.neopets.com/pirates/foodclub.phtml",
          data: data,
          delay: true,
      ...
      _post = function (data, p, cb) {
        page.request({
          method: "post",
          action: "https://www.neopets.com/pirates/process_foodclub.phtml",
          data: data,
          delay: true,
          callback: function (xhr) {
            p && _response(xhr, p);
      ...
      switch (type) {
        case "custom":
          for (var key in FoodClub.templates) {
            try {
              var result = FoodClub.templates[key](
                xhr,
                findArenaBy,
                findPirateBy
              );
              if (
                0 <
                result.list.filter(function (bet) {
                  return (
                    bet.round &&
                    bet.odds &&
                    0 <
                      bet.arenas.filter(function (arena) {
                        return (
                          arena.id &&
                          arena.name &&
                          arena.pirate.id &&
        ...
        case "current":
          ...
          return {
            pirates: xpath(".//a[contains(@href, '=pirates&id=')]/@href", xhr)
              .map(function (o) {
                return _pirate(
                  parseInt(o.value.match(/id=(\d+)/)[1], 10),
                  courses
                );
              })
              .sort(function (a, b) {
        ...
        case "previous":
          return {
            pirate: parseInt(
              xpath(
                "string(.//img[contains(@src, '/fc/fc_')]/@src)",
                xhr
              ).match(/pirate_(\d+)/)[1],
              10
            ),
        ...
        case "bet":
          var pIndex = [],
            ...
            maxbet =
              /max_bet\s*=\s*(\d+)/.test(rawData) && parseInt(RegExp.$1, 10),
          ...
          while ((match = re.exec(rawData))) {
            ...
            if (!(sum % 4)) {
              pirateSelect = xpath(
                ".//select[@name = 'winner" + (1 + arena) + "']",
                xhr.body
              )[0];
              pirateSelect =
                pirateSelect.options[pirateSelect.selectedIndex].value;
              ...
              arenas[arena] = {
                ...
                arbitrage: 100,
                checked: xpath(
                  ".//input[@type = 'checkbox' and @name = 'matches[]' and @value = '" +
                    (1 + arena) +
                    "']",
                  xhr.body
                )[0].checked,
        ...
        case "pirates":
          var stats = xpath(
              ".//td[@class = 'content']/center/table/tbody/tr[td[6][contains(., '%')]]/td[1]/a",
              xhr
            ),
            bets = [],
            pirates = [];
          for (var ai = 0, at = stats.length; ai < at; ++ai) {
            var pi =
                /&id=(\d+)/.test(stats[ai].href) && parseInt(RegExp.$1, 10),
              s = Array.prototype.slice
                .apply(stats[ai].parentNode.parentNode.cells, [1, 5])
                .map(function (item) {
      ...
      for (var ai in arenas) {
        ...
        if (aa.id == arena.id || (!arena.id && aa.name == arena.name)) {
          for (var bi in aa.pirates) {
            ...
            if (
              bb.id == arena.pirate.id ||
              (!arena.pirate.id && bb.name == arena.pirate.name)
            ) {
              return bb;
            }
          }
        ...
        if (b.pirate.id || b.checked) {
          if (data.check) {
            ...
            if (p) {
              ...
              console.error(
                "Pirate not found in the current arena",
                b.pirate.id,
                b.id,
                data.check
              );
