...
// @exclude        *
// @grant          GM_log
// @icon           https://gm.wesley.eti.br/icon.php?desc=142688
  ...
  $(params.clues + " table").each(function (index, node) {
    ...
    $(params.cells, node).each(function (index, node) {
      if (/clue[-_]s(\w)c(\w)/.test(node.getAttribute("class"))) {
        var pos = [
            node.parentNode.cellIndex,
            node.parentNode.parentNode.rowIndex,
          ],
  ...
  (function recursive(obj) {
    if (obj.index < list.length) {
        ...
        if (peace.width + c > 3) {
          ...
          var copy = JSON.parse(JSON.stringify(obj.table)),
            cabe = !peace.nodes.some(function (n) {
              ...
              if (
                (~n.symbol &&
                  (!~copy[i][0] || copy[i][0] == n.symbol) &&
                  (!~copy[i][1] || !~n.color || copy[i][1] == n.color)) ||
                (~n.color &&
                  (!~copy[i][1] || copy[i][1] == n.color) &&
                  (!~copy[i][0] || !~n.symbol || copy[i][0] == n.symbol))
              ) {
                if (~n.symbol) copy[i][0] = n.symbol;
                if (~n.color) copy[i][1] = n.color;
      ...
      table.sort(pieces).forEach(function (p) {
        ...
        ql.add([
          params.negg.colorCell,
          [Math.floor(p.index / 3), p.index % 3],
          [481, 283],
        ]);
      });
      ql.run();
