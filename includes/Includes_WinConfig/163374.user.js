...
// @copyright      2013+, w35l3y (https://gm.wesley.eti.br/includes)
// @license        GNU GPL
...
// @include        nowhere
// @exclude        *
// @icon           https://gm.wesley.eti.br/icon.php?desc=163374
// @grant          GM_log
// @grant          GM_addStyle
// @grant          GM_getValue
// @grant          GM_setValue
...
var WinConfig = function (params) {
  this.name =
    ("title" in params && params.title.replace(/[^\w]+/g, "")) || "default";
  ...
  this.buttons = (function (type) {
    switch (type) {
      ...
      case WinConfig.WindowType.CUSTOM:
        return (
          WinConfig.WindowButton.SAVE |
          WinConfig.WindowButton.RESET |
          WinConfig.WindowButton.CANCEL
        );
    }
  })(this.type);
          ...
          function () {
                ...
                if (field.getConfig instanceof Function) {
                  ...
                  for (var bi in field.elements) {
                    ...
                    if (WinConfig.FieldType.GROUP == field.type) {
                      ...
                      if (elem.checked) {
                        if (
                          ...
                          if (WinConfig.FieldFormat.NUMBER & field.format) {
                            ...
                            WinConfig.FieldFormat.BOOLEAN & field.format
                          ) {
                            if (field.name in cfg) {
                          ...
                          if (WinConfig.FieldFormat.BOOLEAN & field.format) {
                            cfg[field.name] =
                              o == "true" || (!isNaN(o) && !!parseInt(o, 10));
                          } else if (
                            WinConfig.FieldFormat.NUMBER & field.format
                          ) {
                      ...
                      WinConfig.FieldType.TEXT == field.type &&
                      WinConfig.FieldFormat.ARRAY & field.format
                    ) {
                      ...
                      WinConfig.FieldType.TEXT == field.type ||
                      WinConfig.FieldType.HIDDEN == field.type ||
                      WinConfig.FieldType.PASSWORD == field.type
                    ) {
                      var x;
                      if (WinConfig.FieldFormat.NUMBER & field.format) {
                        if (elem.value) {
            ...
            if (xClose) {
              if (this.store) {
                GM_setValue(
                  "config-" + this.name,
                  JSON.stringify((config = tmpConfig))
                );
              }
      ...
      r = function (vv, g) {
        ...
        return k in g
          ? vv.length
            ? r(vv, g[k])
            : g[k]
          : d && (vv.length ? r(vv, d[k]) : d[k]);
      };
    if ("group" in this) {
      return this.group in config
        ? r(x, config[this.group])
        : d && r(x, d[this.group]);
    } else {
      return r(x, config);
    }
  ...
  main.setAttribute(
    "class",
    "winconfig winConfig_" +
      this.name +
      " " +
      ["", "error", "explanation", "question", "prompt", "warning", "success"][
        this.type
      ]
  );
  ...
  this.open = function () {
    if (
      !Array.prototype.some.apply(document.querySelectorAll("style"), [
        function (a) {
          return ~a.textContent.indexOf(".winconfig {");
        },
      ])
    ) {
    ...
    main.style.left =
      (this.position[0] == -1
        ? Math.floor(window.innerWidth / 2 - main.offsetWidth / 2)
        : this.position[0]) + "px";
    main.style.top =
      (this.position[1] == -1
        ? Math.floor(window.innerHeight / 2 - main.offsetHeight / 2)
        : this.position[1]) + "px";
  ...
  (function recursive(_parent, body) {
    for (var ai in _parent.fields) {
      ...
      if (fld.key instanceof Function || fld.key in WinConfig.CustomField) {
        var x = new (
          fld.key instanceof Function ? fld.key : WinConfig.CustomField[fld.key]
        )(_this);
        for (var bi in fld) {
          x[bi] = fld[bi];
        }
      ...
      if (
        (WinConfig.FieldType.GROUP == fld.type &&
          ("label" in fld || !fld.nogroup)) ||
        ("value" in fld &&
          fld.value.length > 1 &&
          WinConfig.FieldType.CHECK == fld.type &&
          !fld.nogroup)
      ) {
          ...
          img.addEventListener(
            ...
            (function (field) {
              return function (e) {
                ...
                if ("default" in field) {
                  text.push(
                    "<sub><b>Default value:</b> " + field.default + "</sub>"
                  );
                }
      ...
      if (WinConfig.FieldType.GROUP == fld.type) {
        ...
        if ("default" in fld) {
          ff.appendChild(
            document.createTextNode(
              WinConfig.FieldFormat.ARRAY & fld.format &&
                fld.default instanceof Array
                ? fld.default.join("\n")
                : fld.default
            )
          );
        ...
        if (fld.multiple) {
          ...
          if (fld.unique && WinConfig.FieldFormat.NUMBER & fld.format) {
            ...
            fld.events.change.push(function (e) {
              for (
                var bi = e.target.selectedIndex;
                bi < e.target.options.length;
                ++bi
              ) {
                ...
                if (opt.selected) {
                  for (var ai in this.value) {
                    ...
                    if (opt.value & f.value) {
                      f.elements[0].selected = !(
                        f.value & ~(opt.value & f.value)
                      );
                    }
                  }
                }
        ...
        if (fld.value instanceof Array) {
          for (var bi in fld.value) {
            ...
            if (
              fld.unique &&
              fld.multiple &&
              WinConfig.FieldFormat.NUMBER & fld.format
            ) {
              ...
              input.events.change.push(function (e, field) {
                var flist = document.querySelectorAll(
                  "div.fieldParent_" +
                    this.parent.name +
                    " input[name = '" +
                    e.target.name +
        ...
        if (WinConfig.FieldType.FILE == fld.type) {
          ...
          for (var bi in fld.value) {
            ...
            ff.setAttribute(
              "type",
              ["text", "hidden", "password"][fld.type] || "text"
            );
        ...
        ff.setAttribute(
          "type",
          ["text", "hidden", "password", "file"][fld.type] || "text"
        );
        if ("default" in fld) {
  ...
  title.addEventListener(
    ...
    function (e) {
      drag = [
        true,
        main.offsetLeft - e.pageX,
        main.offsetTop - e.pageY,
        title.style.cursor,
      ];
  ...
  if (this.store) {
    try {
      ...
      if (
        this.force ||
        (!params.keep && GM_getValue(vn) != GM_info.script.version)
      ) {
        this.open();
      }
    } catch (e) {
  ...
  document.addEventListener(
    ...
    function (e) {
      var name =
          (function r(f) {
            return f && f.parent
              ? (f.parent.parent ? r(f.parent) : "") + f.name + "."
              : "";
