...
// @icon           https://gm.wesley.eti.br/icon.php?desc=292725
// @debug          true
// @uso:author     55607
// @uso:script     292725
// ==/UserScript==
...
var Notify = {
  data: {
    ...
    get id() {
      return (
        (/@uso:script\s+(\d+)/m.test(GM_info.scriptMetaStr) && RegExp.$1) ||
        undefined
      );
    },
    get body() {
    ...
    hosts: {
      get "userscripts.org"() {
        return {
          username: (
            document.querySelector("ul.login_status a[href ~= '/home']") || {}
          ).textContent,
        };
      },
    },
  ...
  services: [
    {
      action:
        (/@homepage\s+(.+)/.test(GM_info.scriptMetaStr) &&
          RegExp.$1 + "/contact.php") ||
        null,
    },
    {
      ...
      get params() {
        return {
          "message[user_id]":
            (/@(?:uso:)?author\s+(?:.+?[\/ ])?(\d+)$/m.test(
              GM_info.scriptMetaStr
            ) &&
              RegExp.$1) ||
            null,
          authenticity_token:
            unsafeWindow.auth_token ||
            (/var auth_token = "([\w+=])"/.test(
              GM_xmlhttpRequest({
                method: "get",
      ...
      if (!ow || !(id = this.data.id) || id == ow.id) {
        ...
        (function recursive(index, p) {
          if (index < p.services.length) {
            ...
            alert(
              "An error has occurred. Consider reporting the following text to the administrator:\n\n" +
                b
            );
          }
        })(0, this);
