...
// @contributor    Marti (https://userscripts.org/messages/95655)
// ==/UserScript==
...
JsCode.beautify = function (text) {
  return js_beautify(text.replace(/[“”]/g, '"'), {
    indent_size: 1,
    indent_char: "\t",
  });
};
...
JsCode.deobfuscate = function (text, variables) {
  var s = JsCode.beautify(text)
      .replace(/(?:new )?Array\((.*?)\);?$/gm, function (m, c) {
  ...
  re = /(\w+)\[(\d+)\]/g;
  while ((matches = re.exec(s))) {
    // replaces static arrays
    if (matches[1] in temp_vars) {
      s = s.replace(
        matches[0],
        '"' +
          temp_vars[matches[1]][matches[2]]
            .replace(/"/g, '\\"')
            .replace(/\n/g, "\\n") +
          '"',
        "g"
  ...
  return s
    .replace(/(".+?")\.z\(\)/g, function ($0, $1) {
      return $1.split("").reverse().join("");
    })
    .replace(
      /eval\(function\([^]+?(["'])(.+)\1, (\d+), (\d+), \1(.+?)\1.+?$/gm,
      function (match) {
        // unpacker
        //	https://userscripts.org/scripts/review/161209
        return JsCode.deobfuscate(
          eval("String" + match.slice(4)).replace(/\\/g, "\\\\"),
          variables
        );
      }
    )
    ...
    .replace(/\\u([0-9a-f]{4})/gi, function (x, m) {
    ...
    .replace(/\\x([0-9a-f]{2})/gi, function (x, m) {
    ...
    .replace(/\\0([0-7]{2})/g, function (x, m) {
      // converts octa to string
      switch (m.toUpperCase()) {
        case "42": // (")
          return '\\"';
        case "12": // (line feed)
    ...
    .replace(/\[(["'])(\w+)\1\]\(/g, function (x, p, func) {
      //	["test"]( => .test(
      return "." + func + "(";
    })
    ...
    .replace(/\w+(?:(?:\("[\w\/]+"\))?(?:\[(['"]?)\w+\1\])+)+/g, function (m) {
      // replaces the "array notation" (document["appendChild"]) to the "dot notation" (document.appendChild)
      var output = "",
        match,
        lastIndex = -1,
        re = /\[(['"])(\w+)\1\]/g;
    ...
    .replace(/unzip\((["'])(.+?)\1\)/gi, function (m, p, code) {
      // https://userscripts.org/scripts/review/152277
    ...
    .replace(/(base64)?decode\((["'])(.+?)\2\)/gi, function (m, b, p, code) {
      // https://userscripts.org/scripts/review/51702
      function _decode(a) {
        var b =
          "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789,;.:-_@#°+]}[{^?'=)(/&%$£\"!\\|~<> ";
        var c = [
          "toHmN9g+Z@.:°GIp\"f2,B7i8<!J3ckdK_&/S5F{r$^au[;hs\\Eql%OUn)01VWA£>DMPy'Q(Y-|z~vxL6}w=?RT]4jX#e bC",
          ")TLOM\\%1Gr\"yh]wFb/4.t?xPo{dp°#N-W8}+i&Bcz~3Y20jC<;nef gH(,S^uJ|kRvEaK5'U:I@[XV!A>6_ZD=m7sQ$q9l£",
          "XnW8'b#.}p,cY{ytU3\\kgBw97CDZj/<SFxe@%JVANETI-5~fmHuhl?od1 )^qs&a6LGM(P[$0:r4+\"2£K]=OQ|R_vz;i!°>",
          "hH6z\"x^v1&Q$eq54°iwd@O8%N>uGV=;m~\\!],)cb?kXKf</Ir:SPgjn3£p#yZloR|W_BET('{tU}A7+0FL Ja.DY29CMs[-",
          "7Hi4@LN6\"on0&m£JAWxzrZTkFq#R|asj]U\\~%(5d=_GX-^I2ybO)Ew9?gPY,+<Q/3MpKl;.v}!{°[ VBDecutf$S:1C8>'h",
          "'i.|O^mdCzf°/ZKS>}jyhE5\"9~R%tku_bwUT6JP8s1MGNQ?WBpvF{£[0&!(=-XceV+a]Yorg<LD:);lH24#A\\3Iq 7$n,@x",
          "aq]9rU1v).@=\\;%'TwZfD~IeWgF\"!(-mAtd6:zjG+JRVE3Mh<|SH80k#Li7nP5O£2,Xx_ycuY>4Q&$°Cp}b/Ko?sNl[^{B ",
          "zLIj=|i%#Y6+k£M(9'>nb5CJ\\~l{p!]}/UT8X\"ud37Ao0PH,21E.<ZQ°hetWRBKF-4qs?@r)_OfNw[Sygm^;cVD&xv:G$a ",
          "oHCy5)Y4?=/|(P£BO%lh&ZQLxN>:a;rIJ9s^,$G{t3k~cEgwv_pUF°2jmnzAW\\!#6Td VK7.8\"iq'DX[fS<]@+be0M1}Ru-",
          "giPQ_:AkFz3Y1WUn?,-oJHD>#e5C}xs$£IScaTqy7°B%p;GMZdOR<=v+@|/6(0^K{'r2\\Xmj&l)!bu~tN]wL.f h49[8EV\"",
          "ir@5XZ/-%7tbwul}^pHdY:~?PL,O6\\jyv_;F°)K|.Wq3U#DaMBe4(&Tf>EAG'<z8={!n9[ m1kRNQS2£hxC+]oV$0I\"gJcs",
          "\\smf<C53°n(8B-[u$o]Xjd/w;e!ht9A~MOpJT|vi=cK)'?U7r1zD4I{k_%,g.6SHE0NyVY2P£@>+a:Wl\"Zq# FxLGR}Q^&b",
          "P°<Ae7Q+F~cKXU.ouZI>x|L[0Y 8t^mNEv:1B&2)$VJh-f9sjwdS3R6=%/5\"r\\'l(a}WD!G]g_?T4Hk{My;£Oipqn,z@#Cb",
          "TiE;<9._?Za{S[,/n4q:mK]BwM\"W)oQ'H@Lxv0+g°JUhyYAV>f\\bO£sz-NlC2%It!}8(~R r6X1cGuj#P&$^D5k73dF=pe|",
          "XidAT3/s{,+M.2Pb>4\"OFZ<WUlEQ&(}xC!Y0G:gkJ_eIwqR#pNm']VK otnhr°)£H7u^[?56@%y-|=1\\;~vfcSzBD9j8La$",
          "~p=@s5K6zPhI{/\\SX°DZ>03!G-t}_ujgBR\"wlxe[dqWy^rvMHVY,TN$£&<A2FQ'i.%?7):EL;1f#acb9|4JO]8kU+( nCmo",
        ];
      ...
      function base64decode(a) {
        ...
        while (i < a.length) {
          ...
          if (enc3 != 64) c += String.fromCharCode(chr2);

          if (enc4 != 64) c += String.fromCharCode(chr3);
        }
        c = _utf8_decode(c);
        return c;
      }
      ...
      var c = code.replace(/\\+([^\\])/g, function (m, x) {
        return m.length - x.length == 1 && !/[B]/.test(x)
          ? x
          : m.substr(Math.floor((m.length - x.length) / 2));
      });
      return (
        '"' +
        (b ? base64decode(c) : _decode(base64decode(_decode(c)))).replace(
          /"/g,
          '\\"'
        ) +
        '"'
    ...
    .replace(/_0x[xa-f0-9]+/gi, function (m, offset, s) {
      // replaces encoded variable names to readable ones
      if (!(m in tvars)) {
      ...
      if (typeof variables != "object") {
        ...
        for (var k in variables) {
          if (
            typeof variables[k] == "object" &&
            !tvars.used[k] &&
            variables[k][1].test(line)
          ) {
            tvars.used[k] = true;
    ...
    .replace(/[1Il]{4,}|[0oO]{4,}/g, function (match) {
      if (
        match.replace(/[10]+/g, "").length > 1 &&
        match.replace(/[IoO]+/g, "").length > 1
      ) {
        return match in tident.keys
          ? tident.keys[match]
          : (tident.keys[match] = "identifier_" + tident.next++);
      } else {
        return match;
      }
    })
    .replace(/String\.fromCharCode\(\d+(?:, \d+)*\)/g, function (match) {
