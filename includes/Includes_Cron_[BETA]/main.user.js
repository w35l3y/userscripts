...
// @icon        https://gm.wesley.eti.br/icon.php?desc=includes/Includes_Cron_[BETA]/main.user.js
// ==/UserScript==
    ...
    Task = function (obj) {
      ...
      Object.defineProperties(this, {
        ...
        interval: {
          enumerable: true,
          value: (
            (obj.interval || "*").replace(/^@(\w+)$/, function ($0, $1) {
                ...
                return {
                  ...
                  values: RegExp.$1
                    ...
                    .replace(/(\d+)-(\d+)/g, function ($0, $1, $2) {
                      var opts = [
                          parseInt($1, 10) +
                            (obj.relative && step ? currentValue % step : 0),
                        ],
                        s =
                          _type.ASTERISK == wildcard && step && !obj.relative
                            ? step
                            : 1;

                      for (
                        var ai = opts[0] + s, at = parseInt($2, 10);
                        ai <= at;
                        ai += s
                      ) {
                        opts.push(ai);
                    ...
                    .filter(function (data, i, array) {
                      return (
                        data != array[i - 1] &&
                        (_type.YEAR == index ||
                          ((_type.MONTH != index || isFinite(--data)) &&
                            pInterval[0] <= data &&
                            data <= pInterval[1]))
                      ); // repeated values and out of bound
                    }),
                };
              }

              throw (
                "Malformed or not supported parameter: " + data + " " + index
              );
            }),
        },
      });
                ...
                if (_type.WEEKDAY == index) {
                  ...
                  date.setUTCDate(
                    pInterval.step -
                      dd +
                      date.getUTCDate() +
                      (obj.relative ? value : tmp[1]) * (dd >= pInterval.step)
                  );
                } else {
                  date["set" + method[3]](
                    pInterval.step + (obj.relative ? value : tmp[1])
                  );
                }
              } else if (
                _type.SLASH == pInterval.type ||
                _type.SECOND == index
              ) {
                if (_type.WEEKDAY == index) {
              ...
              if (_type.WEEKDAY == index) {
                ...
                if (!obj.relative) {
                  ...
                  for (var ci = 0; ci < index; ++ci) {
                    if (_type.WEEKDAY != ci) {
                      date["set" + intervals[ci][3]](
                        this.interval[ci].values[0]
                      );
                    }
                  }
                }
      ...
      for (var index = 0, at = tasks.length; index < at; ++index) {
        ...
        if (pInterval <= _curr) {
          if (
            ...
            (isFinite(o.priority) && t.priority === undefined)
          ) {
    ...
    _updateTimer = function () {
      ...
      if (tasks.length) {
        ...
        timer = setTimeout(function () {
          ...
          if (wait2 < 30000) {
            // tries to solve concurrent executions
            console.log("A task is being executed in another TAB", wait2);
            setTimeout(
              _updateTimer,
              30000 - wait2 + Math.floor(500 * Math.random())
            );
          } else {
