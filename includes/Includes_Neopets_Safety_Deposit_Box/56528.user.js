...
SDB.convert = function (doc) {
  var msg = doc.evaluate(
    ...
    page = doc.evaluate(
      ".//td[@class='content']//td/form/select",
      doc,
      null,
      XPathResult.FIRST_ORDERED_NODE_TYPE,
      null
    ...
    obj = {
      current:
        (page &&
          parseInt(page.options[page.selectedIndex].textContent, 10) - 1) ||
        0,
      last:
        (page &&
          parseInt(page.options[page.options.length - 1].textContent, 10) -
            1) ||
        0,
      error: msg ? 1 : 0,
      message: msg,
...
SDB.list = function (params) {
  if (typeof params.onsuccess != "function") {
    alert(
      "[Includes : Neopets : Safety Deposit Box : list]\nParameter 'onsuccess' is wrong/missing."
    );
  } else {
    ...
    HttpRequest.open({
      method: "get",
      url: "https://www.neopets.com/safetydeposit.phtml",
      headers: {
...
SDB.remove = function (params) {
  ...
  for (var ai = params.items.length; ai--; )
    if (parseInt(params.items[ai][0], 10))
      data["back_to_inv[" + params.items[ai][0] + "]"] =
        parseInt(params.items[ai][1], 10) || 0;
  ...
  HttpRequest.open({
    method: "post",
    url: "https://www.neopets.com/process_safetydeposit.phtml?checksub=scan",
    headers: {
      Referer: "https://www.neopets.com/safetydeposit.phtml",
    },
    onsuccess: function (params) {
...
SDB.removeOne = function (params) {
  if (!params.id) {
    alert(
      "[Includes : Neopets : Safety Deposit Box : removeOne]\nParameter 'id' is wrong/missing."
    );
  } else {
    var obj = params || {};
