...
var StockMarket = function (page) {
  var _this = this,
    ...
    _post = function (data, p, cb) {
      page.request({
        method: "post",
        action: "https://www.neopets.com/process_stockmarket.phtml",
        referer: "https://www.neopets.com/stockmarket.phtml",
        ck: "_ref_ck",
        data: data,
        delay: true,
    ...
    _get = function (data, p, cb) {
      page.request({
        method: "get",
        action: "https://www.neopets.com/stockmarket.phtml",
        data: data,
        delay: true,
        callback: function (xhr) {
          p && _response(xhr, p);
    ...
    Portfolio = function () {
      ...
      this.parse = function (xhr) {
        var shares = [],
          total = xpath(
            "id('postForm')/table[1]/tbody/tr[position() = last()]",
            xhr.body
          )[0].cells;

        xpath("id('postForm')/table/tbody/tr/td[1]/img[2]", xhr.body).forEach(
          function (o) {
            var cells = o.parentNode.parentNode.cells,
              d = {
                id:
                  /^(\d+)/.test(o.previousElementSibling.id) &&
                  parseInt(RegExp.$1),
              ...
              s = xpath(
                ".//tr/td/input",
                o.parentNode.parentNode.nextElementSibling
              )
                .map(function (x) {
            ...
            d.group =
              /stockmarket\/(\d+)\./.test(d.image) && parseInt(RegExp.$1, 10);
  ...
  this.stocks = function (obj) {
    ...
    _get(
      ...
      function (xhr) {
        return {
          list: xpath(
            ".//td[@class = 'content']//table/tbody/tr/td[a/b]",
            xhr.body
          ).map(function (o) {
            var cells = o.parentNode.cells,
              d = {
                id:
                  /&company_id=(\d+)/.test(
                    o.firstElementChild.getAttribute("href")
        ...
        if (list.length) {
          ...
          list
            .filter(function (o) {
              return (
                !obj.list ||
                !obj.list.length ||
                obj.list.some(function (x) {
                  if (
                    x.ticker == o.company.ticker &&
                    (!x.id || x.id == o.id) &&
                    0 <= x.amount
                  ) {
          ...
          _post(
            ...
            function (xhr) {
              return {
                transactions: xpath(
                  ".//center/table/tbody/tr[1 < position()]",
                  xhr.body
                ).map(function (o) {
