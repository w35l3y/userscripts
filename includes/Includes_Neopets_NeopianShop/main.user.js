...
var NeopianShop = function (page) {
		...
		GM_xmlhttpRequest({
			...
			"onload"	: function (xhr) {
				...
				img.addEventListener("load", function (e) {
					...
					var r = cbxy(ctx.getImageData(0, 0, e.target.width, e.target.height));
				...
				img.src = "data:image/jpg;base64," + btoa(String.fromCharCode.apply(null, new Uint8Array(xhr.response)));
	...
	parseHaggle = function (o) {
		var captcha = xpath("string(.//input[contains(@src, 'captcha_show.phtml?_x_pwned')]/@src)", o.body);
		if (captcha) {
			o.result = {
				status		: 0,	// haggle
				captcha		: captcha,
				message		: xpath("normalize-space(.//td[@class = 'content']/font/b[4]/text())", o.body),
				price		: parseInt(xpath("normalize-space(.//td[@class = 'content']/font/b[contains(., \"'\") and position() = last()]/text())", o.body).replace(/\D+/g, ""), 10),
				found		: !xpath("boolean(.//td[@class = 'content']/form/center/div/font/font)", o.body),
				sold		: false
			};
		} else if (xpath("boolean(.//td[@class = 'content']/font/b/b/text())", o.body)) {	// bought
			o.result = {
				status	: 1,	// bought
				item	: {
					name	: xpath("normalize-space(.//td[@class = 'content']/div[2]/b/text())", o.body),
					image	: xpath("normalize-space(.//td[@class = 'content']/div[1]/img[2]/@src)", o.body)
				},
				message	: xpath("normalize-space(.//td[@class = 'content']/div[2])", o.body),
				price	: parseInt(xpath("normalize-space(.//td[@class = 'content']/font/b/b/text())", o.body).replace(/\D+/g, ""), 10),
				found	: true,
				sold	: true
			};
		} else {	// sold
			o.result = {
				status	: -1,	// sold
				message	: xpath("normalize-space(.//td[@class = 'content']/div/font/b/text())", o.body),
				sold	: true
			};
		}
		return o;
	...
	ShopItem = function (node, shopUrl) {
		var haggleUrl = "http://www.neopets.com/";
		if (/this.href=([^;]+);/.test(node.firstElementChild.getAttribute("onclick").replace(/['+]+/g,""))) {
			haggleUrl += RegExp.$1;
		} else {
			throw "Wrong url";
		}
		...
		Object.defineProperties(this, {
			...
			image	: {
				value	: node.firstElementChild.firstElementChild.src.replace(/^http:\/\/images\.neopets\.com\/items\/|\.gif$/g, "")
			},
			stock: {
				value	: texts[0]
			},
		...
		this.haggle = function (obj) {
			if (obj.force || !latestHaggle.captcha) {
				...
				_get(haggleUrl, shopUrl, function (o) {
					parseHaggle(o);
				...
				locateCaptcha(latestHaggle.captcha, haggleUrl, function (result) {
					...
					_post(haggleUrl, haggleUrl, result, function (o) {
						parseHaggle(o);
						latestHaggle = o.result;
						obj.callback.call(_this, o);
					}, obj.delay);
				}, obj.captcha || function (imgdata) {
					var low = [0, 0, 120],
					...
					curr = new Date();
					for (var xi = 0, xt = imgdata.width - rnd;xi < xt;xi += 4, xxi += xi * (xi % 8?1:-1)) {
						for (var yi = 0, yyi = mdy;yi < yt;yi += 4, yyi += yi * (yi % 8?1:-1)) {
							var index = 4 * (xxi + yyi * imgdata.width),
							avg = Math.floor((imgdata.data[index] + imgdata.data[1 + index] + imgdata.data[2 + index]) / 3);	// (red+green+blue)/3
	...
	this.list = function (obj) {
		var url = "http://www.neopets.com/objects.phtml?type=shop&obj_type=" + obj.id,
		curr = new Date();
		...
		_get(url, url, function (o) {
			...
			var missing = [],
			tmpKey = "",
			items = xpath(".//form[@name = 'items_for_sale']//td[@class = 'contentModuleContent']//td[a[img]]", o.body);

			obj.callback({result:{
				message	: (items.length?items.length + " items found.":xpath("normalize-space(.//td[@class = 'content']/center[2]/text())", o.body)),
