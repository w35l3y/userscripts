...
Neopets.convert = function (doc) {
	return new (function (_d) {
		...
		this.Timezone = function () {
			...
			if (tz != 8) {
				console.log([tz, c.getUTCHours(), o.nh, 12 * ("pm" == o.na), c.getUTCMinutes(), o.nm]);
			}
			...
			if (isNaN(tz)) {
				...
				if (i <= c && c < f) {	// daylight saving time
					tz -= 60;
				}
			}
		...
		this.Language = function () {
			return (/var nl = ([""''])?(\w{1,})\1\;$/m.test(_s(".//script[contains(text(), 'var nl = ')]/text()")) && RegExp.$2 || _s(".//select[@name = 'lang']/option[@selected]/@value") || /\blang=(\w+)\b/.test(document.cookie) && RegExp.$1 || "en");
		};
		this.Theme = function () {
			return (/\/themes\/([\d\w_]+)/.test(_s(".//link[contains(@href, '/themes/')]/@href | .//img[contains(@src, '/themes/')][1]/@src")) && RegExp.$1 || "000_def_f65b1");
		};
		this.Neopoints = function () {
			return (_n("id('header')//td/a[contains(@href, '?type=inventory')]/text()"));
		};
