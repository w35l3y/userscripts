...
// @include        nowhere
// @exclude        *
// @icon           https://gm.wesley.eti.br/icon.php?desc=87942
// @grant          GM_addStyle
// @grant          GM_getValue
// @grant          GM_setValue
...
/**************************************************************************
    ...
    Based on https://userscripts.org/scripts/show/52251 (by Buzzy)
...
(function () {
  ...
  if (GM_getValue("cfuw_enabled", true)) {
    (function recursive(new_header) {
      var last = Date.parse(
          GM_getValue("cfuw_last_check", "Sat Oct 9 2010 20:10:26 GMT-0300")
        ),
        curr = new Date(),
      ...
      if (curr - interval > last) {
        ...
        try {
          ...
          console.warn(
            'Resource META is required for the script "' +
              GM_info.script.name +
        ...
        if (old_header) {
          GM_setValue(
            "cfuw_last_check",
            (last = new Date(curr - interval + 310000)).toString()
          );
          ...
          function nextstep(new_header) {
            var parent = document.createElement("div"),
              params = {
                ...
                old_version:
                  old_header.version ||
                  old_header["uso:version"] ||
                  I18n.get("cfuw.version.unknown"),
                new_version:
                  new_header.version ||
                  new_header["uso:version"] ||
                  I18n.get("cfuw.version.unknown"),
                history: "",
            ...
            parent.innerHTML = GM_getResourceText("updaterWindowHtml").replace(
              /{([\w.]+)}/g,
              function ($0, $1) {
            ...
            if (params.history.length) {
              xpath(win + "//div[@class='history']")[0].style.visibility =
                "visible";
            }

            if (
              old_header.name + old_header.namespace !=
              new_header.name + new_header.namespace
            ) {
              xpath(
                win + "//a[contains(@class,'install')]"
              )[0].addEventListener(
                "click",
                function (e) {
            ...
            xpath(win + "//a[contains(@class,'close')]").forEach(function (
              btn
            ) {
            ...
            xpath(win + "//td[@class='disable']/input")[0].addEventListener(
              "click",
              function (e) {
          ...
          if (new_header) {
            ...
            GM_xmlhttpRequest({
              ...
              onload: function (xhr) {
                ...
                if (
                  /^2/.test(xhr.status) &&
                  "" + old_header["uso:hash"] != "" + new_header["uso:hash"]
                ) {
