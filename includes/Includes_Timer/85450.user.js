...
Timer = function (obj) {
  ...
  this._type = /^[0-2]$/.test(obj.type)
    ? obj.type
    : Timer.type[this._target > curr ? "FUTURE" : "PAST"];
  ...
  this.toString = function (str) {
    var output = this.current(),
      test_cond = [
        function (is_future) {
          return (
            (!is_future && this._type == Timer.type.FUTURE) ||
            (is_future && this._type == Timer.type.PAST)
          );
        },
      ],
      t = this;
    ...
    return str.replace(
      ...
      function ($0, $1, $2, $3, $4) {
        ...
        if ($2 == "?") {
          // bool & plural
          return ($1 in test_cond && test_cond[$1].call(t, output[$1])) ||
            output[$1] > 1
            ? $3
            : $4
            ? $4
            : "";
        } else if (output[$1].toString().length < $3) {
          // padding
