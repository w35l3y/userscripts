...
/**************************************************************************
    ...
    Based on https://userscripts.org/scripts/version/63649/158447.user.js?format=txt (by Backslash)
  ...
  switch (type) {
    case "play":
      ...
      return {
        ...
        id: parseInt(
          (/\-(\d+)\.\w+$/.test(img.src) ||
            /game_id=(\d+)/.test(location.search)) &&
            RegExp.$1,
          10
        ),
        name:
          xpath("string(id('gr-ctp-feedback-form')/p/b/text())", doc) ||
          xpath(
            "string(id('gr-ctp-feedback-form')/p/b/text()|id('gr-header')//h1/text())",
            doc
          ).replace(/^[-\s]+/g, "") ||
          img.alt ||
        ...
        ratio:
          parseFloat(
            xpath("string(id('gr-ctp-npratio')/div/text()[1])", doc)
              .replace(/,/g, ".")
              .replace(/[^\d.]+/g, "")
          ) || 0,
        featured: !!xpath(
          "string(.//div[contains(@class, 'burst-featured-game')]/@class)",
          doc
        ).length,
        shockwave: !!xpath(
          "string(.//script/text()[contains(., 'NP_getUrl') and contains(., 'shockwave')])",
          doc
        ).length,
        highscores: xpath(
          "id('gr-ctp-hiscores')//ul[position() = last()]/li/span[2]/text()",
          doc
        )
          .map(function (v) {
    ...
    case "play_flash":
      ...
      var flashvars =
        xpath(
          "string(id('gameWrapper')//param[@name='flashvars']/@value|id('gameWrapper')//script[contains(., 'gamePreloader')]/text()|.//script[contains(., '/games/gaming_system/')]/text())",
          doc
        ) ||
        md.gameURL ||
        "";
    ...
    case "process_flash_score":
      var obj = {
        ...
        plist = Array.prototype.map.apply(
          (typeof doc == "string" ? doc : doc.textContent).split("&"),
          [
            function (a) {
              var o = a.split("=", 2);
...
FlashGame.open = function (obj) {
  ...
  if (!obj.url) {
    var url =
      unsafeWindow.NP_getUrl &&
      unsafeWindow.NP_getUrl(params.size, params.quality);
    obj.url =
      (/game_id=(\d+)/.test(url) && RegExp.$1 == tmpx.game && url) ||
      (params.game_id &&
        "/games/play_flash.phtml?va=&game_id=" +
          params.game_id +
          "&nc_referer=&questionSet=&quality=" +
          params.quality);
  }
  if (!obj.referer) {
    params.size = "large";
    ...
    obj.referer = "https://www.neopets.com/games/game.phtml?" + x.substr(1);
  }
  ...
  ["url", "referer"].forEach(function (v) {
    if (!obj[v]) {
      ...
      obj[v] = "https://www.neopets.com" + obj[v];
    }
  });
...
FlashGame.url = function (obj) {
  ...
  if (nan.length) {
    ...
    if (
      ...
      FlashGame.cached_includes[obj.include].Decimals in decimals_arr
    ) {
    ...
    var next = function (obj) {
      ...
      if (obj.session === true) {
        ...
        FlashGame.session = obj.session = window.setInterval(
          function (obj) {
            HttpRequest.open({
              method: "get",
      ...
      if (i && i.Decimals in decimals_arr && decimals_arr[i.Decimals].length) {
        var encode = {
          ...
          frmrt:
            obj.options.f ||
            (obj.options.f = 24 + Math.floor(12 * Math.random())),
        ...
        var o = "",
          ...
          t = {
            asp_fs_g:
              obj.options.forceScore != undefined
                ? obj.options.forceScore || 0
                : "",
            r: idv.r,
            gmd_g: obj.game,
        ...
        if (obj.beep && !obj._beep) {
          ...
          obj._beep =
            xpath(".//audio[contains(@src, '/beep-8.wav')]")[0] ||
            document.createElement("audio");
          obj._beep.setAttribute(
            "src",
        ...
        obj.start = function (obj) {
          ...
          (function recursive(obj) {
            ...
            if (
              obj.beep instanceof RegExp &&
              obj.beep.test(Math.floor(ms / 1000)) &&
              obj._beep.play
            ) {
              obj._beep.play();
            }
    ...
    if (
      FlashGame.includes[obj.include] &&
      FlashGame.includes[obj.include].Decimals in decimals_arr &&
      obj.cache &&
      !obj.captcha
    ) {
          ...
          onsuccess: function (xhr) {
            ...
            if (
              ...
              obj.onerror({
                code: 0xa008,
                message: [
                  "An error has occurred while requesting file.",
                  "",
                  "Possible reasons and responsibles:",
                  err(4) + "Wrong captcha (you)",
                  err(2) + "Decoding engine is died (showmycode)",
                  err(1) + "File not found (neopets)",
                ].join("\n"),
                data: xhr.error,
              });
              return;
            ...
            if (!is_error && /^(?:13960|89198|97250)$/.test(include.Vid)) {
              ...
              GM_setValue(
                "includes",
                JSON.stringify(FlashGame.cached_includes)
              );
...
FlashGame.send = function (obj) {
  ...
  HttpRequest.open({
    ...
    onsuccess: function (xhr) {
      ...
      var result =
          FlashGame.convert(xhr.response.text, "process_flash_score") || {},
      ...
      if (result.list.call_external_params) {
        try {
          var data =
            JSON.parse(
              decodeURIComponent(result.list.call_external_params).substr(5)
            ) || [];
            ...
            switch (d.fn) {
              ...
              case "flash_func_trigger":
                ...
                if (
                  d.args &&
                  d.args.func &&
                  typeof unsafeWindow[d.args.func] == "function"
                ) {
...
FlashGame.execute = function (obj) {
  obj.recursive = function (obj) {
    ...
    if (!obj.array_score[1] && obj.array_score[0] == obj.max_score) {
      ...
      var rs = Math.floor(1000 / obj.options.n),
        xrand = function (v, r) {
          ...
          return (
            v +
            r * (min + obj.array_score[1]) * obj.array_score[2] * Math.random()
          );
        };

      obj.score = xrand.apply(
        null,
        obj.ratio_score && obj.options.n ? [rs, 0.15] : [obj.array_score[0], 1]
      );
      ...
      while (obj.score > obj.max_score) {
        ...
        if (obj.ratio_score) {
          if (
            obj.max_score > obj.score &&
            4 < ((obj.max_score - obj.score) * x) / obj.array_score[2]
          ) {
            obj.score += (obj.max_score - obj.score) * x;
          } else if (
            obj.score > obj.array_score[0] &&
            4 < ((obj.score - obj.array_score[0]) * x) / obj.array_score[2]
          ) {
    ...
    if (obj.array_time[0] < 0) {
      obj.time *=
        (obj.ratio_time ? obj.score - obj.max_score : 0) - obj.max_score;
    ...
    if (!(obj.confirm instanceof Function)) {
      obj.confirm = function (obj) {
        ...
        for (var label in obj.params) {
          o.push(
            [I18n.get("inpfg." + label + ".label"), obj.params[label]].join(
              "\t\t"
            )
          );
        }
    ...
    obj.onsuccess = function (obj) {
      ...
      obj.params = {
        ...
        opts: [
          obj.options.dc,
          obj.options.ddNcChallenge,
          obj.options.f,
          obj.options.chall,
          obj.options.multiple,
          obj.options.forceScore,
        ].join("|"),
      };
      if (obj.extra) {
