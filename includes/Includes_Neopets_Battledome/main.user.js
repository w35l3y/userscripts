...
var Battledome = function (page) {
	...
	_request = function (url, data, cb, delay) {
		page.request({
			method	: "POST",
			action	: url,
			referer	: "http://www.neopets.com/dome/arena.phtml",
			data	: data,
			...
			callback: function (ro) {
				var r = ro.body;
				if (r.success) {
					_bd.ajaxData.battleid = _bd.battleid = r.battle.battleid || r.battle.id;
	...
	this.fight = function (data1, cb) {
		...
		serializeIcons = function (icons) {
			...
			for (var ai = 0, at = o.length;ai < at;++ai) {
				...
				output.push(k in icons?(icons[k].type == "%"?(100 * icons[k].value) + "%":icons[k].value):"0");
			}
		...
		ajax = function (data2, callback, delay) {
			...
			_request("http://www.neopets.com/dome/ajax/arena.php", d, callback || function () {}, delay);
		},
		strength = [
		...
		recursive1 = function (plays) {
			_this.startFight(data1, function (oo) {
				...
				ajax({
					...
					equipmentsFound = page.database.queryAll("equipments", {
						query	: function (row) {
							...
							for (var ai = 0, at = itemsFound.length;ai < at;++ai) {
								if (!itemsFound[ai].equipment && itemsFound[ai].ID == row.item_ID) {
									itemsFound[ai].equipment = parseEquipment(row);
									for (var bi = 0, bt = equipments.length;bi < bt;++bi) {
					...
					equipmentsImage = itemsFound.filter(function (item) {
						return item.equipment;
					}).map(function (item) {
						return item.image;
					}),
					equipmentsNotFound = equipments.filter(function (item) {
					...
					recursive2 = function (ro) {
						...
						if (r.success) {
							...
							var bestWeapons = parseItems(r.p1.items).sort(function (x, y) {
									...
									if (wY) {
										...
										if (eX.ID != eY.ID) {
											if (1 > Math.abs(eX.rating - eY.rating)) {
												if (4 > Math.abs(eX.attack.total - eY.attack.total)) {
													if (4 > Math.abs(eX.defense.total - eY.defense.total)) {
														return (eX.rating > eY.rating?-1:1);	// DESC
													}
													return (eX.defense.total > eY.defense.total?-1:1);	// DESC
												}
												return (eX.attack.total > eY.attack.total?-1:1);	// DESC
											}
											return (eX.rating > eY.rating?-1:1);	// DESC
										}
										return 0;
							...
							bestAbility = Object.keys(r.p1.abils).filter(function (item) {
								return !r.p1.abils[item].hasCooldown;
							}).map(parseFloat).sort(function (x, y) {
								var a = abilities[x],
								b = abilities[y];
	...
	this.startFight = function (data, cb) {
		_request("http://www.neopets.com/dome/ajax/startFight.php", {
			type		: 2,
			pet			: data.petName,
