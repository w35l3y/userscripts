...
var proxy = new RemotingProxy(
    "https://www.neopets.com/amfphp/gateway.php",
    "WheelService",
    amf.ObjectEncoding.AMF3
  ),
  ...
  wheels = [
    ...
    },
    {
      id: "2", // excitement
      automatic: true,
      //	referer		: "https://images.neopets.com/wheels/wheel_of_excitement_v3_831fbec8f8.swf?r=60260779",
      interval: function (c) {
    ...
    },
    {
      id: "3", // mediocrity
      automatic: false,
      //	referer		: "https://images.neopets.com/wheels/wheel_of_mediocrity_v2_c4ed41eb31.swf",
      interval: function (c) {
        return c - 40 * 60 * 1000;
      }, // 40 minutes
      execute: function (proxy) {
    ...
    },
    {
      ...
      automatic: false,
      headers: {
        Referer: "https://www.neopets.com/prehistoric/monotony/monotony.phtml",
      },
      interval: function (c) {
        return c - 24 * 60 * 60 * 1000;
      }, // 24 hours
...
proxy.addHandler(
  "spinWheel",
  resultCallback(function (body, ctx) {
    var wheel = ctx.wheel;
    ...
    console.log(
      wheel.message && wheel.message(Math.floor(body.slot, 10)),
      body.reply,
      body.toSource()
    );
  }),
...
(function recursive(list, index) {
  if (index < list.length) {
    ...
    if (
      wheel.automatic &&
      (!la[wheel.id] || wheel.interval(curr) > la[wheel.id])
    ) {
      ...
      window.setTimeout(
        recursive,
        5000 + Math.ceil(5000 * Math.random()),
        list,
        ++index
      );
    } else {
