...
// @icon           https://gm.wesley.eti.br/icon.php?desc=178643
...
// @require        https://pastebin.com/raw/5Ji72UdS
...
var jj = /\/job\/([^\/]+)\/changes$/.test(
    decodeURIComponent(location.pathname)
  ) && [location.origin + "/", RegExp.$1],
  ...
  process = function (jobName, loc, doc, info) {
    ...
    var job = jobs[jobName],
      nn = [
        (jobs[jobName] && jobs[jobName].last) || 0,
        (/\d+/.test(xpath("string(id('main-panel')/h2[1]/a/@href)", doc)) &&
          RegExp["$&"]) ||
          0,
      ],
        ...
        var history = [],
          createInfo = function (obj, b) {
            if (!~obj.added.indexOf(b.block.index)) {
              ...
              help.setAttribute(
                "src",
                "/static/58ced3f3/images/16x16/help.png"
              );
              help.setAttribute("style", "cursor: pointer");

              help = obj.node.parentNode.insertBefore(
                help,
          ...
          getTemplate = function (block, build) {
            ...
            context.block.files.forEach(function (f) {
              f.categoryString = ~f.category
                ? jobInfo.categories[1][f.category][1]
                : jobInfo.categories[0];
            ...
            HttpRequest.open({
              ...
              onsuccess: function (xhr) {
                var blocos = (details[index].blocks = []),
                  ibloco = -1;
                job.status = details[index].status = xpath(
                  "string(id('main-panel')/h1/img/@alt)",
                  xhr.response.xml
                );

                xpath(
                  "id('main-panel')/table/tbody/tr",
                  xhr.response.xml
                  ...
                  if ("pane" == row.getAttribute("class")) {
                    ...
                    if (!~ibloco) {
                      var b = {
                          ...
                          owner: xpath(
                            "string(.//a[contains(@href, '/user/')]/text())",
                            row
                          ),
                          mantis: [],
                          users: [],
                        ...
                        fText = (
                          xpath(
                            ...
                            row
                          )[0] || row.firstElementChild
                        ).innerHTML.replace(/<br ?\/?>/g, "\n"),
                        reType = 1;

                      if (
                        /\s*(?:\[\s*(\w{2,}(?:[\|\.\s,\/-]*\w{2,})*)\s*\])?\s*(?:(?:mantis)?\s*(\w*\d+(?:\s*[\n\|,\/e]\s*\w*\d+)*)\s*:)\s*([^]+)\s*$/.test(
                          fText
                        ) ||
                        ((reType = 2) &&
                          /\s*(?:\[(?:mantis)?\s*(\w*\d+(?:\s*[\|,\/e]\s*\w*\d+)*)\s*\])\s*(?:\[\s*(\w{2,}(?:[\|\.\s,\/-]*\w{2,})*)\s*\])?\s*([^]+)\s*$/.test(
                            fText
                          )) ||
                        /\s*(?:\[(?:mantis)?\s*(\w*\d+(?:\s*[\n\|,\/e]\s*\w*\d+)*)\s*\])?\s*(?:\[\s*(\w{2,}(?:[\|\.\s,\/-]*\w{2,})*)\s*\])\s*([^]+)\s*$/.test(
                          fText
                        )
                      ) {
                        if (1 == reType) {
                        ...
                        b.description = RegExp.$3
                          .replace(/��/g, "?")
                          .trim()
                          .replace(/(\w)(\?+)(\w)/g, function ($0, $1, $2, $3) {
                    ...
                    var f = {
                      type:
                        /\/document_(\w+)/.test(
                          xpath("string(./img/@src)", row.cells[0])
                        ) && types.indexOf(RegExp.$1),
                ...
                WinConfig.init({
                  ...
                  size: ["300px", 0],
                  description:
                    "A build já foi processada.<br /><br />Deseja reprocessá-la?",
                  ...
                  load: function () {
                    if (blocos.length) {
                      ...
                      (function recursive2(blocoIndex) {
                        // lista de blocos
                        if (blocoIndex < blocos.length) {
                          ...
                          WinConfig.init({
                            ...
                            condition: function () {
                              if (
                                bloco.mantis.length &&
                                !~(info.hudson.citation || "")
                                  .split(/\s*,\s*/)
                                  .indexOf(bloco.owner)
                              ) {
                                if (
                                  loc.username == bloco.owner ||
                                  ~(info.hudson.others || "")
                                    .split(/\s*,\s*/)
                                    .indexOf(bloco.owner)
                                ) {
                                    ...
                                    if (mIndex < bloco.mantis.length) {
                                      ...
                                      WinConfig.init({
                                        ...
                                        condition: function (cfg) {
                                          if (
                                            mantis.info.password ||
                                            (cfg.text &&
                                              (!info.requestPassword ||
                                                requestPassw[this.name]))
                                          ) {
                                            return 1;
                                          } else {
                                            return -1;
            ...
            .map(function ($0) {
              var dateArr = function (month, day, apm, yhms) {
                ...
                date =
                  ...
                  (/\d+(?:[\/: ]\d+)+/.test($0.textContent) &&
                    RegExp["$&"].match(/\d+/g)),
                obj = {
                  index:
                    (/#(\d+)/.test($0.textContent) &&
                  ...
                  date:
                    ...
                    undefined,
                  link:
                    loc.host + "job/" + jobName + "/" + $0.getAttribute("href"),
                  job: jobName,
                  node: $0,
                  added: [],
                  status: "Pending",
      ...
      return next(
        ...
        function () {
          WinConfig.init({
            ...
            size: ["300px", 0],
            description:
              "Certifique-se de ter preenchido os dados corretamente.<br />Caso tenha alguma dúvida em algum campo, pressione o ícone (?) próximo ao campo.",
            parent: this,
          });
            ...
            fields: [
              {
                ...
                label: "Urls",
                description:
                  "Páginas do Hudson ou Jenkins separadas por linha.<br /><br /><b>Formato:</b><br />protocolo://usuario@host[:porta]/job1|job2|jobN<br /><b>Exemplo:</b><br />https://w35l3y@domain3.com:8080/job1|job2<br />https://w35l3y@domain4.com/hudson/job3<br /><br />A porta é opcional.",
            ...
            fields: [
              {
                ...
                label: "Urls",
                description:
                  "Páginas do Mantis separadas por linha.<br /><br /><b>Formato:</b><br />protocolo://usuario[:senha]@host[:porta]/[prefixo]<br /><b>Exemplos:</b><br />https://w35l3y:123xpto@domain1.com:8080/<br />https://w35l3y@domain2.com/q<br /><br />A senha é opcional. Caso não a informe, ela será pedida posteriormente.<br />A porta é opcional.<br />O prefixo do mantis é opcional. Duas urls não podem conter o mesmo prefixo. Ele serve para identificar qual sistema o mantis informado faz parte.",
                type: WinConfig.FieldType.TEXT,
                format: WinConfig.FieldFormat.STRING,
                multiple: true,
                help: true,
            ...
            value: [
              {
                ...
                help: true,
                description:
                  "Nunca pergunta pela última build processada e sempre continua o processo de quando parou pela última vez.",
              },
              {
                value: 1,
                label: "Diferente",
                help: true,
                description:
                  "Só pergunta quando a build mais recente for diferente da última que foi processada.",
              },
              {
                value: 2,
                label: "Sempre",
                help: true,
                description:
                  "Sempre pergunta, independente da última build que foi processada.<br /><br />Útil por exemplo quando falta energia ou precisa forçar o reinício da máquina.",
              },
            ],
          },
          {
            ...
            value: [
              ...
              },
              {
                ...
                help: true,
                description:
                  "Pergunta caso builds já processadas sejam detectadas.",
              },
              {
                value: 2,
                label: "Sempre",
                help: true,
                description:
                  "Reprocessa builds já processadas sem perguntar.<br /><br />Não aconselhável pois o histórico é perdido.",
              },
            ],
          },
          {
            ...
            value: [
              {
                ...
                help: true,
                description:
                  "Pergunta pela senha apenas uma vez e a armazena para ser usada porteriormente.",
              },
              {
                value: 1,
                label: "Primeira",
                help: true,
                description:
                  "Pergunta pela senha uma vez a cada execução. A senha é armazenada.",
              },
              {
                value: 2,
                label: "Sempre",
                help: true,
                description:
                  "Pergunta pela senha sempre que ela for exigida. Não armazena a senha.",
              },
            ],
          },
        ],
