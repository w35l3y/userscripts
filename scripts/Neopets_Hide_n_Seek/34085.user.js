...
// @grant          GM_getValue
// @grant          GM_setValue
// @include        https://www.neopets.com/games/hidenseek.phtml
// @include        https://www.neopets.com/games/process_hideandseek.phtml?*
// @include        https://www.neopets.com/games/hidenseek/*.phtml?xfn=
// @include        https://www.neopets.com/games/hidenseek/*.phtml
// @icon           https://gm.wesley.eti.br/icon.php?desc=34085
...
(function () {
  ...
  switch (location.pathname) {
    case "/games/hidenseek.phtml":
      var places = xpath(
        ".//td[@class = 'content']//tr/td/a[img and contains(@onclick, '.phtml?xfn=')]"
      );
          ...
          function (place) {
            location.assign(
              "javascript:void(function () {" +
                place.getAttribute("onclick") +
                "}())"
            );
          },
          rnd,
          places[Math.floor(places.length * Math.random())]
        );
      }
      break;
      ...
      if (/\?p=(\d+)&game=(\d+)/.test(location.search)) {
        ...
        if (href) {
          ...
          setTimeout(
            function (b, next) {
              var url =
                "https://www.neopets.com/games/hidenseek/" + b + ".phtml";
    ...
    default:
      if (/(\d+)\.phtml/.test(location.pathname)) {
        ...
        if (places[board].length < areas.length) {
          ...
          while (
            ~places[board].indexOf(
              (index = Math.floor(1 + areas.length * Math.random()))
            )
          ) {}
