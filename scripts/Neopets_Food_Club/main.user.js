...
// @version        1.3.0
// @language       en
// @include        https://www.neopets.com/pirates/foodclub.phtml?type=bet
// @icon           https://gm.wesley.eti.br/icon.php?desc=scripts/Neopets_Food_Club/main.user.js
// @grant          GM_log
// @grant          GM_addStyle
// @grant          GM_getValue
// @grant          GM_setValue
...
button.addEventListener(
  ...
  function (e) {
    ...
    prompt2(
      "Url with predefined bets",
      GM_getValue(savedUrlKey, "https://www.neopets.com/~"),
      function (cfg) {
        var r = fc.parse("bet"),
          url = cfg.text;

        if (url && r.max_bet) {
          // check whether the food club is open
          fc.get(
            function (o) {
              // retrieve the bets of a custom page
              GM_setValue(savedUrlKey, url);
              var bets = o.response.list
                .sort(function (a, b) {
                  // sort and filter the list of bets
                  if (a.round == b.round) {
                    return 0;
                  }
              ...
              if (bets.length) {
                var row = function (i) {
                  var bet = bets[i] || { arenas: [], odds: 1 },
                    sbets = bet.arenas
                      .map(function (arena) {
                        return (
                          '<span style="color: ' +
                          (fc.check(r.arenas, arena) ? "inherit" : "red") +
                          '"><b>' +
                          arena.name +
                          "</b>: " +
                          arena.pirate.name +
                  ...
                  return (
                    '<td><input type="checkbox" name="bet_index" value="' +
                    i +
                    '" ' +
                    (~sbets.indexOf(": red")
                      ? 'disabled="disabled"'
                ...
                var xWin = WinConfig.init({
                  ...
                  size: ["666px", -1],
                  description:
                    '<table border="1" cellpadding="2" cellspacing="0" align="center"><tr><th><input type="checkbox" checked="checked" name="bet_checkall" value="0,5" /></th><th>Info</th><th>Odds</th><th><input type="checkbox" checked="checked" name="bet_checkall" value="5,10" /></th><th>Info</th><th>Odds</th></tr>' +
                    [0, 1, 2, 3, 4]
                      .map(function (i) {
                  ...
                  load: function (cfg) {
                    ...
                    (function recursive(list, index) {
                      // recursively iterate through the list of bets
                      if (index < list.length) {
                        ...
                        if (success && index) {
                          location.assign(
                            "https://www.neopets.com/pirates/foodclub.phtml?type=current_bets"
                          );
                        }
                      }
                    })(
                ...
                xpath(".//input[@name = 'bet_checkall']", xWin.form).forEach(
                  function (bca) {
                    bca.addEventListener(
                      "click",
                      function (e) {
                        var x = e.target.value.split(",").map(parseFloat);

                        xpath(
                          ".//input[@name = 'bet_index']",
                          xWin.form
                        ).forEach(function (bi) {
