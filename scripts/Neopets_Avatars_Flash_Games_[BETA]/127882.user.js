...
function init(doc) {
  ...
  var games = {
    ...
    id = parseInt(
      doc.id || (/game_id=(\d+)/.test(location.search) && RegExp.$1),
      10
    );
  ...
  if (
    ...
    !doc.link &&
    !isNaN(id) &&
    (id in games || GM_getValue("generic", false)) &&
    (!doc.shockwave || GM_getValue("scoresender", false))
  ) {
    var list = JSON.parse(GM_getValue("games", "{}")),
      usern = xpath(
        "string(.//a[contains(@href, '/userlookup.phtml?user=')]/text())"
      ),
      slist =
        "#gmc" == location.hash
          ? []
              .concat(
                (
                  prompt(
                    "List of games",
                    xpath(".//a[contains(@href, 'game_id=')]/@href")
                      .map(function (x) {
                        return /game_id=(\d+)/.test(x.nodeValue)
                          ? RegExp.$1
                          : null;
                      })
                      .join(",")
      ...
      data_g = function (id) {
        var x = (GM_getValue("stored", true) && games[id]) ||
          list[id] ||
          games[id] || [0, 0, 0, 100];
      ...
      updateGame = function (x) {
        ...
        avatar.attr(
          "src",
          "https://images.neopets.com/neoboards/avatars/" +
            (data[4] || "default.gif")
        );
      ...
      randscore = function (v) {
        var result = [
            Math.ceil(v || data[0] || 1000 * doc.ratio || 0),
            1000 * Math.random(),
          ],
        ...
        result[0] +=
          Math.ceil(Math.min(20743 / (Math.abs(data[2]) * data[3]), data[1])) *
          data[3] *
          Math.random();
        ...
        result[1] +=
          data[2] *
          (1 + 0.1 * Math.random()) *
          (result[0] - (data[2] < 0 ? 2 * (data[0] + data[1] * data[3]) : 0));
      ...
      copy = {
        "gr-ctp-settings-btn": {
          ...
          execute: function (node, p) {
            if (!p) {
              p = xpath(
                ".//a[contains(@href, '/neoboards/boardlist.phtml?board=')]/following-sibling::*[1]"
              )[0];
          ...
          execute: function (node, p) {
            ...
            var td = btn.parentNode.parentNode.cloneNode(true),
              ...
              score_time = randscore(
                /score=(\d+)/.test(location.hash) && RegExp.$1
              ),
              score =
                typeof data[7] == "function"
                  ? data[7](score_time[0])
                  : score_time[0];
            ...
            table.rows[0].innerHTML =
              '<th style="width:75px" class="confirmation-2">Score</th><td style="width:91px" class="confirmation-2"><input name="score" type="text" id="field_score"  value="' +
              score_time[0] +
              '" /></td><td rowspan="2" width="50" height="50" class="confirmation-2"><img id="image_avatar" src="https://images.neopets.com/neoboards/avatars/' +
              (data[4] || "default.gif") +
              '" title="' +
              (data[5] || doc.name || "") +
              '" /></td><td rowspan="6" class="confirmation confirmation-2" style="width:5px">&nbsp;</td><th colspan="2" class="confirmation" id="label_score">' +
              score +
              "</th>";
            table.rows[1].innerHTML =
              '<th class="confirmation-2">Time (ms)</th><td class="confirmation-2"><input type="text" name="time" id="field_time" value="' +
              score_time[1] +
              '" /></td><th colspan="2" class="confirmation" id="label_time"><span class="red">00:00</span></th>';

            var groups = [
              {
                name: "captcha",
                list: [
                  '<th class="confirmation-2"><img src="https://www.showmycode.com/?c" id="image_captcha" /></th><td colspan="2" class="confirmation-2"><input id="field_captcha" name="captcha" maxlength="1" /></td><th class="confirmation">Params</th><td class="confirmation aleft" id="label_opts"><span class="red">0|0|0|||</span></td>',
                  '<th colspan="3" class="acenter confirmation-2">Captcha</th><th class="confirmation">Username&nbsp;</th><td class="confirmation aleft" id="label_username">' +
                    usern +
                    "</td>",
                ],
              },
            ...
            table.insertRow(2).innerHTML =
              '<td colspan="2" class="aleft confirmation-2"><input id="field_cache" name="cache" type="checkbox" value="1" /> <label for="field_cache">Try cached encryption</label></td><td class="confirmation-2"><input type="button" value="R" title="Randomize" id="button_rand" /></td><th class="confirmation" style="width:75px">Game</th><td class="confirmation aleft" style="width:205px" id="label_game">' +
              doc.name +
              " (" +
              id +
              ")</td>";
            ...
            if (!p) {
              table.insertRow(2).innerHTML =
                '<th style="width:75px" class="confirmation-2">Extra</th><td style="width:91px" colspan="2" class="confirmation-2"><input name="extra" type="text" id="field_extra"  value="' +
                obj2url(efn(data[6])(score)) +
                '" /></td><td colspan="2" class="confirmation aleft" style="width:205px" id="label_extra">&nbsp;</td>';
    ...
    for (var c in copy) {
      ...
      if (node) {
        ...
        tmp.setAttribute(
          "id",
          String.prototype.replace.apply(tmp.getAttribute("id"), r)
        );
      } else {
    ...
    if (!doc.shockwave && (!~ss.indexOf(id) || !slistEmpty)) {
      ...
      $(document).ready(function (e) {
        ...
        $("#image_captcha").click(function (e) {
          $(this).attr(
            "src",
            "https://www.showmycode.com/?c#r" + Math.random()
          );
          $("#field_captcha").focus();
        });
        $("#field_score").change(function (e) {
          ...
          $("#field_time")
            .val(
              Math.ceil(
                data[2] *
                  (1 + 0.1 * Math.random()) *
                  (v - (data[2] < 0 ? 2 * (data[0] + data[1] * data[3]) : 0)) +
                  1000 * Math.random()
              )
            )
            .change();
          ...
          if (
            data[3] != mod &&
            (-1 != mods.indexOf(data[3]) || mc & 4) &&
            ((mc & 1 &&
              data[3] > mod &&
              confirm(
                "Mod has changed from " + data[3] + " to " + mod + ". Continue?"
              )) ||
              (mc & 2 &&
                (mod = parseInt(
                  prompt(
                    "Define mod score manually:",
                    data[3] < mod ? mod : data[3]
                  ),
                  10
                )) > 0))
          ) {
        ...
        $("#ctp-avatar-save").click(function (e) {
          ...
          if (!cache && !captcha.length) {
            ...
            FlashGame.execute({
              elements: pg
                ? Array.prototype.slice.apply(pg.elements)
                : {
                    game_id: id,
                    size: "regular",
                    quality: "high",
              ...
              confirm: function (obj) {
                ...
                $("#label_score").html(
                  '<span class="' +
                    (obj.params.score != obj.array_score[0] ? "green" : "") +
                    '">' +
                    obj.params.score +
                    "</span>"
                );
                ...
                $("#label_game").attr(
                  "class",
                  "confirmation aleft" + (id == obj.params.game ? "" : " red")
                );
                $("#label_username").html(
                  '<span class="' +
                    (obj.params.username == usern ? "" : "red") +
                    '">' +
                    obj.params.username +
                    "</span>"
                );
              ...
              onerror: function (e) {
                ...
                if (
                  (0x002 & e.code && "captcha" == e.data) ||
                  (0x008 & e.code && 4 == e.data) ||
                  0x004 & e.code
                ) {
                  $("#image_captcha").click();
                }
              ...
              onsuccess: function (obj) {
                ...
                if (
                  -1 != [0, 3, 11, 26].indexOf(parseInt(obj.list.errcode, 10))
                ) {
                  if (!data[0]) {
                    // min_score
                    data[0] = obj.score;
              ...
              FlashGame.execute({
                ...
                confirm: function (obj) {
                  var test = [
                      !first,
                      obj.plays <
                        (~[3, 5].indexOf(obj.list.ms) ? obj.list.ms : sp),
                      obj.params.score,
                      obj.time > 10000,
                  ...
                  [
                    ...
                    [
                      "game",
                      obj.name +
                        ' (<a target="_blank" href="https://www.neopets.com/games/game.phtml?game_id=' +
                        obj.params.game +
                        '">' +
                        obj.params.game +
                        "</a>)",
                  ...
                  $("#image_avatar")
                    ...
                    .attr("title", data[5] || obj.name || "");

                  if (!test[1]) {
                    $("#ctp-avatar-save-2").hide();
                    $("#ctp-message")
                      .text(I18n.get("npafg.msg.reached_max"))
                      .show();
                    recursive();
                    return 0;
                  } else if (!(test[2] && test[3] && test[6])) {
                    // score, time, params
                    $("#ctp-avatar-save-2").hide();
                    if (0 > obj.tries--) {
                ...
                onsuccess: function (obj) {
                  ...
                  if (-1 != [0, 3, 11, 17, 21, 26].indexOf(err)) {
                    // continue
                    if (
                      -1 != [0, 26].indexOf(err) &&
                      obj.plays <
