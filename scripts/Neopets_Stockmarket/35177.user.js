...
// @icon           https://gm.wesley.eti.br/icon.php?desc=35177
// @connect        neopets.com
// @connect        github.com
...
/**************************************************************************
    ...
    Based on https://userscripts.org/scripts/show/14675 (by nungryscpro)
...
function strftime(format, ts) {
  ...
  ts = new Date(ts * 1000);
  var out = "",
    re = /[^%]?%([aAbBcCdDegGhHIjmMnprRStTuUVwWxXyYzZ%])/g;
  for (var match, i = 0; (match = re.exec(format)); i += 3) {
    out += format.substring(
      Math.max(i - 1, 0),
...
function attachModule(module, prepend) {
  var oldElem = xpath(
    './/td[1]/div[contains(string(table/tbody/tr[1]/td), "' +
  ...
  newElem.innerHTML =
    '<table width="158" cellpadding="0" cellspacing="0" border="0" class="sidebarTable"><tr><td valign="middle" class="sidebarHeader medText">__TITLE__</td></tr>__CONTENT__</table>'
      .replace("__TITLE__", module.title || "")
      .replace("__CONTENT__", module.content || "");

  if (oldElem) {
    xpath("id('content')/table/tbody/tr/td[1]")[0].replaceChild(
      newElem,
      oldElem
    );
  } else if (prepend) {
    xpath("id('content')/table/tbody/tr/td[1]")[0].insertBefore(
      newElem,
      xpath(".//table/tbody/tr/td[1]/div[1]")[0]
    );
  } else {
...
(function () {
  ...
  function executeContent(cont) {
    var content = cont.replace(
      /(?:<\/(?:a|td|font|b|nobr)>| [a-z]+="[#a-z0-9]+"|<(?:b|br|nobr|font)(?:\s*\/)?>|\s+)/gim,
      ""
    );
    ...
    for (var i = 0, t = matches.length; i < t; ++i) {
      var match = matches[i].match(
        />([a-z]+)<ahref="stockmarket\.phtml\?type=profile&(?:amp;)?company_id=(\d+)">.+?<td>(\d+)<td>(\d+)<td><font>([+-]?\d+)<td>(\d+(?:[,.]\d+)*)<td>(\d+(?:[,.]\d+)*)<td>(-?\d+(?:[,.]\d+)*)<td><font>([+-]?\d+[,.]\d+)%/i
      );
      match.shift();
  ...
  function recursive(first) {
    ...
    var nst = new Date(),
      ...
      lastAccess = new Date(
        Date.parse(
          GM_getValue("lastAccess", "Sat Jul 16 2011 09:24:27 GMT-0300")
        ) || 0
      );
    ...
    if (interval <= nst - lastAccess) {
      GM_setValue("refreshing", true);
      lastAccess = nst;
      GM_setValue("lastAccess", nst.toString());

      HttpRequest.open({
        method: "get",
        url: "https://www.neopets.com/stockmarket.phtml?type=portfolio",
        onsuccess: function (xhr) {
          GM_setValue("refreshing", false);
      ...
      wait = setInterval(
        function () {
          if (!GM_getValue("refreshing", false)) {
            addModule(
              JSON.parse(GM_getValue("companies", "[]")),
              JSON.parse(GM_getValue("portfolio", "[]"))
            );
            clearInterval(wait);
          }
        },
  ...
  function addModule(companies, portfolio) {
    var pp = GM_getValue("prepend", -1);
    attachModule(
      {
        title: GM_getResourceText("stockmarketModuleHtml").replace(
          /__([A-Z]+)__/g,
          function ($0, $1) {
            switch ($1) {
              case "TITLE":
                return user.translate[0];
              case "OPTIONS":
                return user.translate[7];
              case "BUYPRICE":
                return user.translate[8];
              case "SELLPRICE":
                return user.translate[9];
              case "CLOSE":
                return user.translate[10];
              case "MINIMUM":
                return user.translate[11];
              case "MAXIMUM":
                return user.translate[12];
              case "BUYMIN":
                return GM_getValue("buyMinimum", 15);
              case "BUYMAX":
                return GM_getValue("buyMaximum", 20);
              case "SELLMIN":
                return GM_getValue("sellMinimum", 30);
              default:
                return $0;
            }
          }
        ...
        content: (function () {
          var out =
            '<tr><td class="activePet sf"><img id="stockmarketRefreshImage" width="150" height="150" border="0" src="https://images.neopets.com/images/' +
            user.image +
            '" style="cursor: pointer;" /><span id="stockmarketRefreshSpan">' +
            strftime(
              "%Y-%m-%d %T",
              Date.parse(
                GM_getValue("lastAccess", "Sat Jul 16 2011 09:24:27 GMT-0300")
          ...
          companies.sort(function (a, b) {
            // sort by holdings
            var x = parseInt(
              (getInfo(portfolio, 1, a[0])[5] || "0").replace(/[,.]/g, ""),
              10
            );
            var y = parseInt(
              (getInfo(portfolio, 1, b[0])[5] || "0").replace(/[,.]/g, ""),
              10
            );
            if (x == y) return 0;
            return x > y ? 1 : -1;
          ...
          // BUYS
          out +=
            '<tr><td class="activePet sf" style="padding: 2px;"><strong>' +
            user.translate[1] +
            '</strong></td></tr><tr><td class="activePet sf"><table width="100%" cellspacing="0" cellpadding="2" border="0" class="activePetInfo">';
          out +=
            '<thead><tr style="background-color:#EFEFEF;"><th style="text-align:left;">' +
            user.translate[3] +
            '</th><th style="text-align:middle;">' +
            user.translate[4] +
            '</th><th style="text-align:right;">' +
            user.translate[5] +
            "</th></tr></thead><tbody>";
          var buy = [
            GM_getValue("buyMinimum", 15),
            GM_getValue("buyMaximum", 20),
          ];
          for (var i = 0, t = companies.length; i < t; ++i) {
            ...
            out +=
              ...
              '" id="stockmarketBuy' +
              companies[i][1] +
              '" onmouseover="this.style.backgroundColor=\'#FFFFD5\';" onmouseout="this.style.backgroundColor=\'' +
              color +
              '\';"><td style="text-align: left;cursor: pointer;">' +
              companies[i][1] +
              '</a></td><td style="cursor: pointer;">' +
              companies[i][2] +
              '</td><td style="text-align: right;cursor: pointer;">' +
              (getInfo(portfolio, 1, companies[i][0])[5] || 0) +
              "</td></tr>";
          }
          ...
          out +=
            '<tr><td class="activePet sf" style="padding: 2px;"><strong>' +
            user.translate[2] +
            '</strong></td></tr><tr><td class="sf"><table width="100%" cellspacing="0" cellpadding="2" border="0" class="activePetInfo">';
          out +=
            '<thead><tr style="background-color:#EFEFEF;"><th style="text-align:left;">' +
            user.translate[3] +
            '</th><th style="text-align:middle;">' +
            ...
            user.translate[5] +
            "</th></tr></thead><tbody>";
          var sell = [GM_getValue("sellMinimum", 30)];
          for (
            var i = 0, t = portfolio.length;
            i < t && parseInt(portfolio[i][3], 10) >= sell[0];
            ++i
          ) {
            var color =
              parseInt(portfolio[i][3], 10) < 15 ? "#FFF5F5" : "#FFFFFF";
            out +=
              '<tr bgcolor="' +
              color +
              '" id="stockmarketSell' +
    ...
    xpath(".//tr[contains(@id, 'stockmarketBuy')]").forEach(function (elem) {
      elem.addEventListener(
        ...
        function (event) {
          ...
          location.replace(
            "/stockmarket.phtml?type=buy#ticker_symbol=" +
              company +
              "&amount_shares=1000"
          );
          xpath(
            ".//td[@class = 'content']//form/table/tbody/tr/td/input[@type = 'text']"
    ...
    xpath(".//tr[contains(@id, 'stockmarketSell')]").forEach(function (elem) {
      elem.addEventListener(
        ...
        function (event) {
          ...
          location.replace(
            "/stockmarket.phtml?type=portfolio#" + company + "disclosure"
          );

          if (/\/stockmarket\.phtml\?type=portfolio/.test(location.href)) {
            var x = GM_getValue("lastSell");
            if (x && company != x) {
              location.replace(
                "javascript:disclose('" + x + "');disclose('" + company + "');"
              );
            }
          }
          GM_setValue("lastSell", company);
        },
        false
      );
    });
    xpath("id('stockmarketOption')")[0].addEventListener(
      "click",
      function (event) {
    ...
    xpath("id('stockmarketFormOption')")[0].addEventListener(
      "submit",
      function (event) {
        GM_setValue("buyMinimum", event.target.elements[0].value || 15);
        GM_setValue("buyMaximum", event.target.elements[1].value || 20);
    ...
    xpath("id('stockmarketRefreshImage')")[0].addEventListener(
      "click",
      function (event) {
  ...
  if (/\/stockmarket\.phtml\?type=portfolio/.test(location.href)) {
    GM_setValue("lastAccess", new Date().toString());
