...
// @version     1.1.0
// @language    en
// @include     https://www.neopets.com/games/neoquest/neoquest.phtml*
// @grant       GM_log
// @grant       GM_addStyle
// @grant       GM_getValue
// @grant       GM_setValue
...
// @icon        https://gm.wesley.eti.br/icon.php?desc=scripts/Neopets_NeoQuestI_Helper/main.user.js
...
GM_addStyle(
  ".winConfig_NeoQuestHelperSettings .fieldClass_hotkey {float:left;margin-right:2px;width:33%}"
);
  ...
  var _this = this,
    executa = function (o) {
      ...
      if (!_this.loading) {
        ...
        block.setAttribute(
          "style",
          "background-color:#000;position:absolute;width:" +
            v.clientWidth +
            "px;height:" +
            v.clientHeight +
            "px;opacity:0.3"
  ...
  this.refresh = function () {
    ...
    xpath(obj.targets, view).forEach(function (target) {
      ...
      if (
        ...
        !(p instanceof HTMLTableSectionElement)
      ) {
        // workaround due to some malformed forms
...
var area = new ViewMonitor({
    ...
    targets:
      ".//a[contains(@href, 'neoquest.phtml') and not(contains(@href, '../'))]|.//area[contains(@href, 'neoquest.phtml')]|.//form[contains(@action, 'neoquest.phtml')]",
  }),
  findActions = function (act, node) {
    return xpath(
      ".//a[contains(@onclick, 'setdata(') and contains(@onclick, '" +
        act +
        "')]",
      node
    )
      ...
      .map(function (val) {
        return (
          /', (\d+)/.test(val.getAttribute("onclick")) && [
            true,
            {
              method: "post",
              action: "https://www.neopets.com/games/neoquest/neoquest.phtml",
              elements: {
                fact: act,
                type: RegExp.$1,
              },
  ...
  groups = [
    {
      ...
      callback: function (node, key, index) {
        if (4 === index) {
          return xpath(
            ".//table/tbody/tr/td[3]/table/tbody/tr[2]/td/a|.//form[@method = 'post' and contains(@action, 'neoquest.phtml')]/input[@type = 'submit']",
            node
          )[0];
        } else {
          return xpath(
            ".//area[contains(@href, '?action=move&movedir=" +
              [1, 2, 3, 4, null, 5, 6, 7, 8][index] +
              "')]",
            node
          )[0];
        }
      ...
      keys: [
        ...
        [
          ...
          function (node) {
            var current = xpath("./div/div[2]/b[3]", node)[0],
              total = xpath(
                "substring(string(./following-sibling::text()[1]), 2)",
                current
              ),
            ...
            var item =
              diff &&
              findActions("item", node)
                .filter(function (val) {
                  return !!val[1].data.text.indexOf("(heals ");
                })
                .map(function (val) {
                  val[1].data.heals =
                    /\(heals (\d+)\)/.test(val[1].data.text) &&
                    parseInt(RegExp.$1, 10);
                  val[1].data.qnty =
                    /\((\d+) left\)/.test(val[1].data.text) &&
                    parseInt(RegExp.$1, 10);
        ...
        [
          ...
          function (node) {
            return (
              findActions("attack", node)[0] || [
                false,
                {
                  href: "https://www.neopets.com/games/neoquest/neoquest.phtml",
                },
              ]
            );
          },
    ...
    },
    {
      ...
      callback: function (node, key, index) {
        return xpath(
          ".//a[contains(@href, '?movetype=" + (1 + index) + "')]",
          node
        )[0];
      },
      keys: [
