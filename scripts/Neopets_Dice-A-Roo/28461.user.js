...
// @copyright      2011+, w35l3y (https://www.wesley.eti.br)
// @license        GNU GPL
...
// @icon           https://gm.wesley.eti.br/icon.php?desc=28461
...
// @cfu:version    version
// @history        6.0.0 Added <a href="https://userscripts.org/guides/773">Includes Checker</a>
...
(function () {
  ...
  if (options) {
    ...
    (function recursive(obj) {
      var btn = xpath(
        ".//td[@class = 'content']//div[2]//form//input[@type = 'submit']"
      );
      ...
      if (btn.length) {
        var iff,
          options_tmp = {
            dice: (/\/dice\/((\D+)\d+)/.test(
              xpath(
                "string(.//td[@class = 'content']//div/center/img[1][not(following-sibling::img[1]) and contains(@src, '/games/dice/')]/@src)"
              )
            ) && [
              ~(iff = dice.indexOf(RegExp.$1)) ? iff : dice.indexOf(RegExp.$2),
              RegExp.$1,
            ]) || [-1, undefined],
            amount:
              (btn.length > 1 &&
                parseInt(
                  btn[1].value.replace(/[,.]+/g, "").match(/\d+/),
                  10
                )) ||
              0,
            stat: ~(
              (/\/(\w+)\/blumaroo_(\w+)_/.test(
                xpath(
                  "string(.//td[@class = 'content']//div/center/img[contains(@src, '_baby.gif')]/@src)"
                )
              ) &&
          ...
          sf = function (f) {
            ...
            HttpRequest.open({
              ...
              onsuccess: function (xhr) {
                ...
                var module = xpath(
                    ".//td[@class = 'content']//div[@class = 'frame']"
                  )[0],
                  nmodule = xpath(
                    ".//td[@class = 'content']//div[@class = 'frame']",
                    xhr.response.xml
        ...
        if (
          (~options.stop.dice &&
            ~options_tmp.dice[0] &&
            (1 << options_tmp.dice[0]) & options.stop.dice &&
            last.dice &&
            options_tmp.dice[0] != last.dice[0] &&
            ~(stop = 0)) ||
          (~options.stop.turn &&
            options_tmp.turn >= options.stop.turn &&
            options_tmp.turn != last.turn &&
            (stop = 1)) ||
          (~options.stop.time &&
            delta.time >= options.stop.time &&
            (stop = 2)) ||
          (~options.stop.amount &&
            options_tmp.amount >= options.stop.amount &&
            options_tmp.amount > last.amount &&
            (stop = 3))
        ) {
          ...
          wait = window.setTimeout(
            ...
            options.interval.min +
              Math.floor(options.interval.rnd * Math.random()),
            btn[0].form
          );
        }
