...
// @icon           https://gm.wesley.eti.br/icon.php?desc=scripts/Trello_Print_board/main.user.js
// @grant          GM_openInTab
// @grant          GM_getResourceText
// @resource       core https://d78fikflryjgj.cloudfront.net/css/76ead370f4805199a6d67aca7c014143/core.css
// @resource       normalize https://fiddle.jshell.net/css/normalize.css
// @resource       print-boardCss https://pastebin.com/raw/gpNrTUkh
// @require        https://code.jquery.com/jquery-2.1.3.js
// @require        https://api.trello.com/1/client.js?key=05e1b25a9057bb2aecbc99363285a8c2&dummy=.js
// ==/UserScript==
    ...
    if (/\/b\/(\w+)/.test(location.pathname)) {
      ...
      Trello.boards.get(
        ...
        function (board) {
          ...
          var members = {},
            $ulList = $("<ul>"),
            $check = $("<input type='checkbox'>").addClass("checkbox"),
            $hr = $("<hr>").addClass("hr");
          ...
          $.each(board.lists, function (ix, list) {
            ...
            $.each(
              board.cards.filter(function (card) {
              ...
              function (ix, card) {
                var $liCard = $("<li>")
                  ...
                  .append(
                    $("<div>")
                      .addClass("badge badge-state-image-only subscribed")
                      .append(
                        $("<div>").addClass(
                          "badge-icon icon-sm" +
                            (card.subscribed ? " icon-star" : "")
                        )
                      )
                  )
                  .append(
                    ...
                    $.each(checklist.checkItems, function (ix, item) {
                      var $liItem = $("<li>")
                        ...
                        .prepend(
                          $check
                            .clone()
                            .prop("checked", "complete" == item.state)
                        )
                        .addClass("item")
                        .appendTo($ulItem);
                    });
          ...
          if (openInBackground) {
            $("#print-document").contents().find("body").html($ulList);

            !debug && $("#print-document")[0].contentWindow.print();
          } else {
...
$("#content").one("DOMNodeInserted", "#permission-level", function (e) {
  ...
  if (openInBackground) {
    ...
    debug &&
      $("#print-document").css("width", "800px").css("height", "400px").show();
      ...
      $("#print-document")
        ...
        .append(
          $("<style>", {
            ...
            .append(GM_getResourceText("core"))
            .append(GM_getResourceText("normalize"))
            .append(GM_getResourceText("print-boardCss"))
        );
