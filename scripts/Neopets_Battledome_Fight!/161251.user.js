...
// @include        https://www.neopets.com/dome/arena.phtml#
// @icon           https://gm.wesley.eti.br/icon.php?desc=161251
// @connect        github.com
...
// @history        2.1.1 Added <a href="https://userscripts.org/guides/773">Includes Checker</a>
...
setTimeout(function () {
  var plays = GM_getValue("plays", 0),
    ...
    opp = new WinConfig({
      ...
      load: function () {
        unsafeWindow.location.replace(
          "https://www.neopets.com/dome/arena.phtml"
        );
      },
      default: players["@"] || {
      ...
      fields: [
        {
          ...
          description:
            "URLs of the abilities.<br /><br />Add them in the order that you would like to use, even if you don't have them.<br />They will be ignored in case they can't be used for some reason.<br /><sup>Separate them by row</sup>",
          multiple: true,
          help: true,
        },
        {
          ...
          format: WinConfig.FieldFormat.STRING | WinConfig.FieldFormat.ARRAY,
          description:
            "URLs of the weapons.<br /><br />Add them in the order that you would like to use, even if you don't have them.<br />They will be ignored in case they can't be used for some reason.<br /><sup>Separate them by row</sup>",
          multiple: true,
          help: true,
        },
      ],
      ...
      fields: [
        ...
        },
        {
          ...
          callback: function () {
            if (
              1 < plays &&
              confirm("Would you like to stop playing the next fights?")
            ) {
              plays = 1;
            }
          },
          ...
          fields: [
            {
              ...
              label: "Step",
              description:
                "Time of waiting between steps<br /><sub><i>Time in miliseconds</i></sub>",
            ...
            },
            {
              ...
              label: "Reload",
              description:
                "Time of waiting before a forced page reload<br /><sub><i>Time in miliseconds</i></sub>",
              format: WinConfig.FieldFormat.NUMBER,
              help: true,
            },
            {
          ...
          fields: [
            {
              ...
              label: "Opponent",
              description:
                "Set of weapons/abilities for each opponent separately.",
            ...
            },
            {
              ...
              label: "Reset",
              description:
                "Auto reset the set of weapons/abilities for the current opponent.<br /><br /><sub><i>This will reset your current settings of XXX vs YYY.<br />Continue?</i></sub>",
              type: WinConfig.FieldType.CHECK,
              format: WinConfig.FieldFormat.BOOLEAN,
              multiple: true,
              help: true,
            },
            {
              ...
              label: "Default",
              description:
                "Auto update the default set of weapons/abilities.<br /><br /><sub><i>Update your default weapons and abilities too?</i></sub>",
              type: WinConfig.FieldType.CHECK,
              format: WinConfig.FieldFormat.BOOLEAN,
              multiple: true,
              help: true,
    ...
    execute = function (k) {
      var r =
          0 < plays ||
          0 < (plays = prompt("How many times to fight?", config.play)),
        player = players[k],
        list = [
          ["#start:visible"],
          [
            ...
            function () {
              $("<img />", {
                ...
                style:
                  "position:absolute;z-index:14;top:112px;left:8px;display:none;",
              }).insertBefore("#p1hp");
      ...
      if (r) {
        ...
        (function recursive2(list, x, round) {
          function el(
            ...
            if (selectedItems && selectedItems.length) {
              ...
              for (var bi = 0; bi < spaceListLength; ++bi) {
                for (
                  var ci = itemsPosition[type];
                  ci < selectedItems.length;
                  ++ci
                ) {
                  ++itemsPosition[type];

                  if (checkAvailabilityOfItem(selectedItems[ci])) {
                    var spaceCssSelector =
                      "#" +
                      spaceIdsList.shift().getAttribute("id") +
                      (spaceAvailable || "") +
                      ":visible";

                    list.push(
                      [spaceCssSelector, 0, spaceInUse, [spaceCssSelector]],
                      [
                        itemListId +
                          " img[src $= '" +
                          selectedItems[ci] +
          ...
          el(
            0,
            "#p1am:has(div:not([style *= 'background-image:'])),#p1am:has(div:not([style *= 'opacity: 1']))",
            "#p1ability",
            player.abilities,
            function (b) {
              // 4
              return (
                $(
                  "#p1ability td:not([style *= 'display: none']) img[src $= '" +
                    b +
                    "']"
                ).length > 0
              );
          ...
          el(
            1,
            "#p1e1m:has(div:not([style *= 'background-image:'])), #p1e1m:has(div:not([style *= 'opacity: 1'])), #p1e2m:has(div:not([style *= 'background-image:'])), #p1e2m:has(div:not([style *= 'opacity: 1']))",
            "#p1equipment",
            player.weapons,
            function (b) {
              // 4
              return (
                $(
                  "#p1equipment li:not([style *= 'display: none']) img[src $= '" +
                    b +
                    "']"
                ).length > 0
              );
          ...
          var tmp =
            "#fight.caction:not(.inactive):visible, button.end_ack:visible";
          list.push([
            tmp,
            0,
            function recursive3(l, x, p) {
              if (x.hasClass("caction")) {
                l.unshift(
                  [
                    "#skipreplay:not(.replay):visible",
                    0,
                    function () {
                      if (
                        config.scroll &&
                        $("#logcont:not(.collapsed)").length
                      ) {
                        scroll("#skipreplay");
                      }
                  ...
                  [
                    ...
                    0,
                    function (l, x, p) {
                      return (x.hasClass("caction") ? recursive2 : recursive3)(
                        l,
                        x,
                        p
                      );
  ...
  if (config && !(key in players && execute(key))) {
    ...
    if (
      !(
        ...
        !(key in players) &&
        (!config.p2name ||
          confirm("Would you like to use your default weapons and ability?")) &&
        execute(key2)
      )
    ) {
      if (!(key in players)) {
        ...
        !config.reset &&
        !confirm(
          "This will reset your current settings of " +
            p1name +
            " vs " +
            p2name +
            ".\nContinue?"
      ...
      $(".ability,.item").live("click", function (e) {
        if (first) {
          ...
          if (
            !(key2 in players) ||
            config.def ||
            confirm("Update your default weapons and abilities too?")
          ) {
        ...
        var img = xpath(".//img", e.target.parentNode)[0],
          s = img.getAttribute("src").replace("https://images.neopets.com", ""),
