...
// @icon        https://gm.wesley.eti.br/icon.php?desc=scripts/GitHub_Missing_Readme/main.user.js#
...
if (
  /^\/(\w+)\/(\w+)\/tree\/(\w+)\/(.+)/.test(location.pathname) &&
  confirm("Update README.md?")
) {
  var info = {
      Username: RegExp.$1,
      Reponame: RegExp.$2,
  ...
  if (!~info.Path.indexOf("/")) {
    repo.getTree(info.Branch + "?recursive=true", function (err, tree) {
      var scripts = [],
        path = {
          name: info.Path,
          fname:
            info.Path.charAt(0).toUpperCase() +
            info.Path.substr(1).toLowerCase(),
            ...
            scripts.push({
              ...
              sshots: blobs
                .filter(function (a) {
                  return (
                    !a.path.indexOf(root + "/") &&
                    !~a.path.indexOf("/", 1 + root.length) &&
                    /\.(?:jpg|png|gif)$/i.test(a.path)
                  );
                })
                .map(function (a) {
      ...
      (function recursive(ai) {
        if (ai < scripts.length) {
          repo.getBlob(scripts[ai].file.sha, function (err, data) {
            ...
            scripts[ai].group = scripts[ai].meta.name
              .split(" : ")
              .slice(0, -1)
              .filter(function (a) {
                return a != path.fname;
    ...
    var files = Array.prototype.slice.apply(
      document.querySelectorAll("td.content a")
    );
